  view: T3aaS_Deactivation_Billing_Adjustments_Totals {
    derived_table: {
      sql:
...
--     DATE_DEACTIVATED
-- from
--     financial_systems.t3_saas_silver.stg_analytics_t3_saas_billing__deactivation_billing_adjustments
select
        concat(deacts.SERIAL_FORMATTED,'-', deacts.DEACTIVATION_TICKET_ID,'-', deacts.DEACTIVATION_TYPE) as UNIQUE_IDENTIFIER,
        TO_VARCHAR(companies.name) as COMPANY_NAME,
        deacts.COMPANY_ID as COMPANY_ID,
        deacts.BUNDLE_TYPE,
        deacts.SERIAL_FORMATTED,
        TO_VARCHAR(deacts.SALES_REF_ID) as SALES_REF_ID,
        deacts.DEACTIVATION_TICKET_ID,
        deacts.DEVICE_TYPE,
        deacts.DEACTIVATION_TYPE,
        deacts.DEACTIVATION_DATE
    from
        analytics.t3_saas_billing.deactivation_billing_adjustments deacts
    left join
        es_warehouse.public.companies companies
        on deacts.COMPANY_ID = companies.COMPANY_ID
)
...
, cameras_to_exclude as (
    select
        SERIAL_FORMATTED
    from
        analytics.t3_saas_billing.deactivation_billing_adjustments
    where
        bundle_type = 'TELEMATICS SERVICE TRACKER & CAMERA BUNDLE' and DEVICE_TYPE = 'Camera'
        or bundle_type = 'TELEMATICS SERVICE TRACKER & CAMERA BUNDLE' and DEVICE_TYPE = 'T3Camera'
)

, deactivation_billing_adjustment_units as (
    select
        COMPANY_NAME,
        COMPANY_ID,
        BUNDLE_TYPE,
        SALES_REF_ID,
        count(UNIQUE_IDENTIFIER) as DEACTIVATION_BILLING_ADJUSTMENT_UNITS
    from deactivations
    group by
        COMPANY_NAME,
        COMPANY_ID,
...
select
    CONCAT(COMPANY_ID,' - ',SALES_REF_ID,' - ',BUNDLE_TYPE) as UNIQUE_IDENTIFIER,
    COMPANY_NAME,
    COMPANY_ID,
    BUNDLE_TYPE,
    SALES_REF_ID,
    DEACTIVATION_BILLING_ADJUSTMENT_UNITS
from
    deactivation_billing_adjustment_units
    ...
    dimension: DEACTIVATION_BILLING_ADJUSTMENT_UNITS {
      type: number
      sql: ${TABLE}.DEACTIVATION_BILLING_ADJUSTMENT_UNITS ;;
    }
