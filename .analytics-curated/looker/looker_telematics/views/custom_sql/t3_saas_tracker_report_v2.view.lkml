view: t3_saas_tracker_report_v2 {
...
with showcase_fulfillments_mapping as (
-- temporary mapping of showcase fulfillments to contracts, need to build out table or data capture method --
select distinct
    FK_SALES_REF_ID,
    case
        when FK_SALES_REF_ID = '45092484712' then '46257104692'
    else FK_SALES_REF_ID
    end as SF_SALES_REF_ID
from
    financial_systems.t3_saas_gold.warehouse_shipment_data
)

, warehouse_shipment_data_dbt_all as (
-- DBT MART Warehouse Shipment Data --
select
    warehouse_shipment_data.SN_UNIQUE_IDENTIFIER,
    warehouse_shipment_data.LINKED_DEVICE_TYPE,
    warehouse_shipment_data.PART_DESCRIPTION,
    warehouse_shipment_data.SHIPPED_SERIAL_NUMBER,
    warehouse_shipment_data.SHIPPED_SERIAL_FORMATTED,
    warehouse_shipment_data.WAREHOUSE_PHYSICAL_DATE,
    warehouse_shipment_data.WAREHOUSE_REQUESTED_SHIP_DATE,
    warehouse_shipment_data.INVOICE_LINE_SUM,
    warehouse_shipment_data.INVOICE_SALES_PRICE,
    warehouse_shipment_data.INVOICE_ID,
    warehouse_shipment_data.SALES_ORDER_NUMBER,
    showcase_fulfillments_mapping.SF_SALES_REF_ID as FK_SALES_REF_ID,
    warehouse_shipment_data.COMPANY_NAME_WHS,
    warehouse_shipment_data.HUBSPOT_PIPELINE,
    warehouse_shipment_data.DATE_DEACTIVATED,
    warehouse_shipment_data.FK_DEACTIVATION_TICKET_ID,
    row_number()
            over (partition by warehouse_shipment_data.SHIPPED_SERIAL_FORMATTED order by warehouse_shipment_data.WAREHOUSE_PHYSICAL_DATE)
    as SERIAL_NUMBER_SHIPPED_INSTANCE
from
    financial_systems.t3_saas_gold.warehouse_shipment_data
left join
    showcase_fulfillments_mapping
    on warehouse_shipment_data.FK_SALES_REF_ID = showcase_fulfillments_mapping.FK_SALES_REF_ID
)
...
left join
    hubspot_contract_data_adj
    on shipment_to_contract.FK_SALES_REF_ID::TEXT = hubspot_contract_data_adj.SALES_REF_ID::TEXT
...
, showcase_fulfillment_shipment_to_contract_mapping as (
select
    showcase_fulfillments_mapping.SF_SALES_REF_ID::TEXT as FK_SALES_REF_ID,
    warehouse_shipment_data.SHIPPED_SERIAL_FORMATTED,
    hubspot_contract_data_adj.CONTRACT_NAME,
    warehouse_shipment_data.LINKED_DEVICE_TYPE,
...
left join
    hubspot_contract_data_adj
    on showcase_fulfillments_mapping.SF_SALES_REF_ID::TEXT = hubspot_contract_data_adj.SALES_REF_ID::TEXT
    and warehouse_shipment_data.LINKED_DEVICE_TYPE = hubspot_contract_data_adj.LINKED_DEVICE_TYPE
)
...
select
    *
from
showcase_fulfillment_shipment_to_contract_mapping
)
...
select
    CONCAT('UPGRADE', SALES_REF_ID, ' - ', COMPANY_ID) as UNIQUE_IDENTIFIER,
    ...
    CONTRACT_QUANTITY as BINARY_BILLING_UNIT
from
    hubspot_contract_data_adj
where
...
select
    ...
    legacy_tracker_report.ASSET_ID,
...
select distinct
    ...
    legacy_tracker_report_mapped.ASSET_ID,
    ...
    hubspot_contract_data_adj.CONTRACT_TERMS_IN_MONTHS,
    hubspot_contract_data_adj.CONTRACT_TERMS_END_DATE
from
...
-- Looker: warehouse_shipment_data_adj
, legacy_tracker_report_hubspot_contract_data_adj as (
select
    LI_ID,
    TRUNC(COMPANY_ID) as COMPANY_ID,
    COMPANY_NAME,
    ...
    CONTRACT_CLOSE_DATE,
    CONTRACT_TERMS_IN_MONTHS,
    MONTHS_REMAINING_ON_CONTRACT,
    null as CONTRACT_TERMS_END_DATE,
    CONTRACT_INSTALL_TYPE,
    CONTRACT_BUNDLE_INSTALL_STATUS,
    ACCT_EXEC_ID as ACCT_EXEC_HS_ID,
    ACCT_EXEC_NAME,
    ACCT_EXEC_EMAIL,
    'Invoice Installed Units' as BINARY_CUSTOMER_BILLING_DESIGNATION,
    FIRST_ADJUSTMENT_ID,
...
, hubspot_contract_data_adj_looker as (
select
    *
from
    legacy_tracker_report_hubspot_contract_data_adj
union
select
    LI_ID,
    TRUNC(COMPANY_ID) as COMPANY_ID,
...
-- Looker: legacy_tracker_report_v2
select
    DEVICE_SERIAL,
    VENDOR,
    CONFIG,
...
from
  ...
  dimension: CONTRACT_TERMS_END_DATE {
    type: date
    sql:  ${TABLE}.CONTRACT_TERMS_END_DATE ;;
  }
