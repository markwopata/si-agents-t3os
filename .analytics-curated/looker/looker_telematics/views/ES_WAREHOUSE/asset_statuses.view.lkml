view: asset_statuses {
  ...
  dimension: dead_batteries {
    type: string
    sql: CASE WHEN
            (((${battery_voltage} > .1 and ${battery_voltage} < 10.5)  and assets.battery_voltage_type_id = 1) or
            ((${battery_voltage} > .1 and ${battery_voltage} < 21) and assets.battery_voltage_type_id = 2))
            THEN 'Yes'
            ELSE 'No'
            END;;
  }
