view: asset_status_key_values {
  sql_table_name: "ES_WAREHOUSE"."PUBLIC"."ASSET_STATUS_KEY_VALUES"
    ;;
  ...
  # Still a work in progress - PB
  # dimension: latitude {
  #   type: location
  #   sql: st_y(to_geography(${TABLE}.value)) WHERE LOWER(${name}) = 'location' ;;
  # }

  # dimension: longitude {
  #   type: location
  #   sql: st_x(to_geography(${TABLE}.value)) WHERE LOWER(${name}) = 'location' ;;
  # }
  ...
  measure: count {
    type: count
    drill_fields: [asset_status_key_value_id, name]
  }
}
