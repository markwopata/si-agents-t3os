view: heap_app_path {
  derived_table: {
    sql: with previous_90_day_users as (
      ...
      concat(app_ranking_usage,' - ',heap_event) as ranking_and_event
      from
      ...
      fs.heap_event,
      fs.usage_percent,
      fs.app_ranking_usage,
      fs.ranking_and_event,
      hui.company_id,
      lag(fs.ranking_and_event) over (partition by fs.heap_user_id order by fs.usage_percent) as next_path_value,
      lag(fs.usage_percent) over (partition by fs.heap_user_id order by fs.usage_percent) as next_path_value_usage
      from
      final_select fs
      left join heap_user_info hui on hui.heap_user_id = fs.heap_user_id
      ;;
  }
  ...
  dimension: ranking_and_event {
    type: string
    sql: ${TABLE}."RANKING_AND_EVENT" ;;
  }
  ...
  dimension: next_path_value {
    type: string
    sql: ${TABLE}."NEXT_PATH_VALUE" ;;
  }

  dimension: next_path_value_usage {
    type: string
    sql: ${TABLE}."NEXT_PATH_VALUE_USAGE" ;;
  }
