view: intacct_vendors {
  sql_table_name: "INTACCT"."VENDOR" ;;
  ...
  dimension_group: msa_valid_through {
    type: time
    timeframes: [raw, date, week, month, quarter, year]
    convert_tz: no
    datatype: date
    sql: ${TABLE}."MSA_VALID_THROUGH" ;;
  }
  dimension: name {
    type: string
    sql: ${TABLE}."NAME" ;;
  ...
  dimension: ft_book_of_business {
    type: string
    sql: ${TABLE}."FT_BOOK_OF_BUSINESS" ;;
  }
  dimension: ft_category {
    type: string
  ...
  dimension: ft_fleet_track_id {
    type: string
    sql: ${TABLE}."FT_FLEET_TRACK_ID" ;;
  }
  dimension: ft_core_designation {
    type: string
