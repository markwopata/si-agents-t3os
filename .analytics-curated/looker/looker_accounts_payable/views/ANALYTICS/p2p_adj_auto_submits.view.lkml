...
where request_id not in (select request_id from analytics.concur.deleted_invoices)
...
FROM (
 ...
 UNION ALL
  ...
  SELECT
    ...
    COUNT(*) AS row_count
  FROM
      counts where adjusted_exclusion_reasons is  not null
  GROUP BY
    month
...
select *, processed_invoices_count - system_concur_processed_invoices_count as ap_rep_submitted, adjusted_invoices_received_count + system_concur_processed_invoices_count as adjusted_clean_x
from oldy
  ;;
  }
  ...
  dimension: ap_rep_submitted {
    type: number
    sql: ${TABLE}."AP_REP_SUBMITTED" ;;
  }
  dimension: adjusted_clean_x {
    type: number
    sql: ${TABLE}."ADJUSTED_CLEAN_X" ;;
  }
