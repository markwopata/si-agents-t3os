view: time_and_fin_ute_by_class {
  derived_table: {
    sql:
            Select
            dt.END_DATE as date,
...
--             h.in_fleet_oec as in_fleet_oec_sum
        ...
        FROM FLEET_OPTIMIZATION.GOLD.UTILIZATION_ASSET_HISTORICAL h
        left join FLEET_OPTIMIZATION.GOLD.DIM_TIMEFRAME_WINDOWS_HISTORIC dt on dt.TF_KEY = h.TF_KEY
        left join FLEET_OPTIMIZATION.GOLD.DIM_AGG_HISTORIC_REVENUE_ASSET r on r.AGG_REV_CALCULATION_KEY = h.AGG_REV_CALCULATION_KEY and r.TF_KEY = h.TF_KEY
...
--     sum(oec_adjusted) as oec_adjusted_sum,
...
-- group by 1,2,3
  ...
  measure: revenue_sum {
    type: sum
    value_format: "$#,##0"
    sql:  ${TABLE}.revenue_sum ;;
  }

  measure: oec_adjusted_sum {
    type: sum
    value_format: "$#,##0"
    sql:  ${TABLE}.oec_adjusted_sum ;;
  }

  measure: rental_oec_sum {
    type: sum
    value_format: "$#,##0"
    sql:  ${TABLE}.rental_oec_sum ;;
  }

  measure: in_fleet_oec_sum {
    type: sum
    value_format: "$#,##0"
    sql:  ${TABLE}.in_fleet_oec_sum ;;
  }
