view: fleet_track_data {
  derived_table: {
    ...
    WITH main AS (
      SELECT DISTINCT
        ...
        POL.SALES_TAX,
        POL.AFTERMARKET_OEC,
        COALESCE(POL.NET_PRICE, 0) + COALESCE(POL.FREIGHT_COST, 0) + COALESCE(POL.SALES_TAX, 0) - COALESCE(POL.REBATE, 0) AS OEC,
        COALESCE(OEC, 0) + COALESCE(POL.AFTERMARKET_OEC, 0) AS TOTAL_OEC,
        POL.INVOICE_NUMBER,
        POL.INVOICE_DATE,
  ...
  dimension: serial_vin {
    ...
    sql:
    CASE
      WHEN ${TABLE}.PREFIX IN ('S', 'B') THEN ${TABLE}.SERIAL
