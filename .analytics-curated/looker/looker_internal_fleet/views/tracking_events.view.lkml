view: tracking_events {
  sql_table_name: "PUBLIC"."TRACKING_EVENTS"
    ;;
  drill_fields: [tracking_event_id]
  ...
  measure: latest_tracking_event_id {
  type: number
  sql: max(${tracking_event_id}) ;;
  }

  dimension: latest_tracking_event_id2 {
    type: number
    sql: ${tracking_event_id} in (select max(${tracking_event_id}) from tracking_events group by ${asset_id}) ;;
  }
