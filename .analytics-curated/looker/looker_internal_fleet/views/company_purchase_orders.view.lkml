view: company_purchase_orders {
  ...
  dimension: category_type {
    type: string
    sql: CASE
        WHEN ${company_purchase_order_type_id} = 1 THEN 'Industrial Jobsite Equipment & Tools'
        WHEN ${company_purchase_order_type_id} = 3 THEN 'Vehicles'
        ELSE 'Other'
        END ;;
  }
  ...
  measure: count {
    type: count
    drill_fields: [company_purchase_order_id, company_purchase_order_types.company_purchase_order_type_id, company_purchase_order_types.name, company_purchase_order_line_items.count]
  }
}
