...
view: asset_purchase_history_facts_final {
  ...
  measure: OEC {
    ...
    value_format: "$#,##0"
    drill_fields: [assets.asset_id,equipment_assignments.start_date,assets.make_and_model,assets.name, OEC, equipment_charge ]
  }

  measure: equipment_charge {
    label: "Equipment Charge"
    description: "Branch earnings charge for this asset"
    type: sum
    sql: ${latest_purchase_price} * 0.014849 ;;
    value_format: "$#,##0"
    drill_fields: [assets.asset_id,equipment_assignments.start_date,assets.make_and_model,assets.name, OEC, equipment_charge ]
  }
  ...
  measure: unavailable_oec {
    ...
    drill_fields: [asset_status_key_values.value, OEC]
  }
  ...
  measure: unavailable_oec_service {
    ...
    sql: ${latest_purchase_price};;
    value_format: "$#,##0"
    drill_fields: [asset_link,asset_status_key_values.value,latest_purchase_price]
#changing sql from CASE WHEN ${asset_status_key_values.value} IN ('Make Ready','Needs Inspection', 'Soft Down','Hard Down') then ${latest_purchase_price} ELSE null END to using a filter instead so that drill down is also filtered.
#     filters: {
...
#     }
    ...
    filters: [
      ...
      asset_status_key_values.value: "Pending Return,Make Ready,Needs Inspection,Soft Down,Hard Down"
    ]
    #taking out is public rsp filter due to it not being present in the denominator or any other metric on service dashboard
  ...
  measure:  unavailable_oec_perc{
    ...
    type: number
    sql: CASE WHEN ${asset_purchase_history_facts_final.OEC} >0 THEN (${asset_purchase_history_facts_final.unavailable_oec} / ${asset_purchase_history_facts_final.OEC}) * 100 ELSE 0 END ;;
    value_format: "0.0\%"
  }

  measure:  unavailable_oec_perc_html{
    description: "%age of unavailable assets"
    type: number
    sql: CASE WHEN ${asset_purchase_history_facts_final.OEC} >0 THEN (${asset_purchase_history_facts_final.unavailable_oec} / ${asset_purchase_history_facts_final.OEC}) * 100 ELSE 0 END ;;
    value_format: "0.0\%"
    html: {{unavailable_oec._rendered_value}} unavailable of {{OEC._rendered_value}} total OEC ;;
    drill_fields: [market_region_xwalk.market_name, unavailable_oec]
  }
  measure:  unavailable_oec_percent_service{
    description: "%age of unavailable assets"
    type: number
    sql: CASE WHEN ${asset_purchase_history_facts_final.OEC} >0 THEN (${asset_purchase_history_facts_final.unavailable_oec_service} / ${asset_purchase_history_facts_final.OEC}) * 100 ELSE 0 END ;;
    value_format: "0.0\%"
    drill_fields: [market_region_xwalk.market_name, unavailable_oec_service]
  }
  ...
  measure:  unavailable_oec_percent_service_as_percentage{
    ...
    type: number
    value_format: "0.0\%"
    sql: CASE WHEN ${asset_purchase_history_facts_final.OEC} >0 THEN (${asset_purchase_history_facts_final.unavailable_oec_service} / NULLIF(${asset_purchase_history_facts_final.OEC}, 0)) * 100 ELSE 0 END ;;
    html: {{unavailable_oec_service._rendered_value}} unavailable of {{OEC._rendered_value}} total OEC ;;
    drill_fields: [market_region_xwalk.market_name, unavailable_oec_service]
  }
