view: market_class_inventory_status_count {
  derived_table: {
    ...
    sql: with asset_info as (
      ...
      from
        ...
        left join ES_WAREHOUSE.PUBLIC.ASSETS a on aa.asset_id = a.asset_id
        -- change join to markets to be based on only rental branch id
        ...
        left join es_warehouse.public.business_segments bs on ec.business_segment_id = bs.business_segment_id
      where
       {% condition market_region_xwalk.market_name %} m.name {% endcondition %}
        and  ais.current_flag = 1
        --and a.asset_type_id = 1 --Andrew Lowe had us remove this and a.company_id and add m.company_id. --Jack G. 8/6/21
        and a.deleted = false
        ...
        AND {% condition make_filter %} aa.make {% endcondition %}
        AND {% condition model_filter %} aa.model {% endcondition %}
      group by
        m.market_id,
        ec.name,
        ec.category_id,
      ...
      select
        ma.class_name,
        coalesce(sum(assetcount),0) as ttl_assets_in_class
      from
 ...
 ;;
  ...
  dimension: market_class_pkey {
    primary_key: yes
    type: string
    sql: CONCAT(${market_id}, '-', ${class_name}, '-', ${category_id}) ;;
  }
  ...
  measure: total_asset_count {
    type: sum
    sql: ${onrentcount}+${readytorentcount}+${assignedcount}+${unassignedcount}+${harddowncount}+${makereadycount}+${needsinspectioncount}+${pendingreturncount}+${predeliveredcount}+${softdowncount}+${onrpocount} ;;
  }
  ...
  measure: total_asset_oec {
    type: sum
    sql: ${onrentoec}+${readytorentoec}+${assignedoec}+${unassignedoec}+${harddownoec}+${makereadyoec}+${needsinspectionoec}+${pendingreturnoec}+${predeliveredoec}+${softdownoec}+${onrpooec} ;;
    value_format_name: usd_0
  }
