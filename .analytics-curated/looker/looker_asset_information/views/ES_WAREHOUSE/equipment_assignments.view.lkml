view: equipment_assignments {
  ...
  parameter: days_out {
    ...
    type: number
    default_value: "7"
    allowed_value: { label: "Next 3 Days" value: "3" }
    allowed_value: { label: "Next 7 Days" value: "7" }
  }

  filter: within_days_out {
    sql:
    ${start_date} BETWEEN CURRENT_DATE()
    AND DATEADD('day', {% parameter days_out %}, CURRENT_DATE()) ;;
  }


  dimension: within_3_days {
    type: yesno
    sql: ${start_date} BETWEEN CURRENT_DATE() AND DATEADD('day', 3, CURRENT_DATE()) ;;
  }

  dimension: within_7_days {
    type: yesno
    sql: ${start_date} BETWEEN CURRENT_DATE() AND DATEADD('day', 7, CURRENT_DATE()) ;;
  }
  ...
  set: detail {
    fields: [
      ...
      requested_by_user.email_address,
