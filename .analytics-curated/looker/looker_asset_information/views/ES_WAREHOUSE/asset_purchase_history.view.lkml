view: asset_purchase_history {
  ...
  measure: count {
    type: count
    drill_fields: [assets.driver_name, assets.asset_id, assets.custom_name, assets.name]
  }

  dimension: order_number_fleet {
    type: string
    sql: coalesce(${TABLE}.purchase_order_url, ${TABLE}.order_number);;
  }

  dimension: invoice_number_fleet {
    type: string
    sql: coalesce(${TABLE}.asset_invoice_url, ${TABLE}.invoice_number);;
  }

  dimension: total_purchase_price_dim {
    type: number
    value_format_name: usd_0
    sql: coalesce(${TABLE}.oec, ${TABLE}.purchase_price);;
  }

  measure: total_purchase_price {
    type: sum
    sql: coalesce(${TABLE}.oec, ${TABLE}.purchase_price);;
    filters: [total_purchase_price_dim: "<>0"]
  }

  # dimension: purchase_created_date {
  #   type: date
  #   sql: coalesce(${invoice_purchase_date},${assets.created_date}) ;;
  # }

  measure: total_purchase_price_dd {
    type: sum
...
assets.make,
assets.model,
assets.factory_build_specs,
assets.serial_number_vin,
aph_vendor.market_id,
aph_vendor.market_name,
aph_vendor.invoice_date,
assets.purchase_created_date,
aph_vendor.due_date,
aph_vendor.title_status,
abl_category.abl_category,
asset_id,
aph_vendor.vendor_name,
invoice_number_fleet,
aph_vendor.reconciliation_status,
total_purchase_price,
aph_vendor.order_status,
assets.company_id,
aph_vendor.due_days_buckets_monthly,
aph_vendor.days_until_due,
purchase_order_number,
aph_vendor.vendor_id,
assets.company_id,
aph_vendor.approval_status,
aph_vendor.latest_approved_date,
aph_vendor.week_to_be_paid,
aph_vendor.recon_status_w_statment_verification,
aph_vendor.fleet_calculation_oec,
aph_vendor.release_date,
aph_vendor.customer_paid_date,
aph_vendor.customer_paid,
aph_vendor.book_of_business,
aph_vendor.category,
aph_vendor.core_designation,
aph_vendor.financing_designation,
aph_vendor.sage_vendor_id,
aph_vendor.note
  ...
  measure: filter_for_processors_total_purchase_price_dd {
    type: sum
    drill_fields: [filter_for_processors_fleet_details*]
    sql: coalesce(${TABLE}.oec, ${TABLE}.purchase_price);;
    filters: [total_purchase_price_dim: "<>0"]
  }
  set: filter_for_processors_fleet_details {
    fields: [
      order_number_fleet,
      aph_vendor.vendor_name,
      assets.make,
      assets.model,
      aph_vendor.net_price,
      aph_vendor.market_name,
    ]
  }
}
