view: rental_rate_by_company {
  ...
  dimension: quoted_price_per_day {
    type: number
    sql: ${TABLE}."PRICE_PER_DAY" ;;
  }

  dimension: quoted_price_per_month {
    type: number
    sql: ${TABLE}."PRICE_PER_MONTH" ;;
  }

  dimension: quoted_price_per_week {
    type: number
  ...
  dimension: view_active_rental_contracts {
    ...
    link: {
      label: "View Active Rentals by Customer"
      url: "https://equipmentshare.looker.com/dashboards-next/238?Asset+ID=&District={{ _filters['market_region_xwalk.district_text'] | url_encode }}&Market={{ _filters['rental_rate_by_company.market_name'] | url_encode }}&Region={{ _filters['market_region_xwalk.region_name'] | url_encode}}&Salesperson={{ _filters['users.Full_Name_with_ID'] | url_encode}}"
    }
  }

  measure: count {
    type: count
    drill_fields: [salesperson,rental_id, equipment_class,asset_id, market_name, company_name,start_date,end_date , amount]

    # link: {
    #   label: "View Active Rental Contracts"
