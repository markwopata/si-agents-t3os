...
view: company_avg_page_load {
  derived_table: {
    sql: SELECT
          ...
          domain,
          query
          -- ROUND(AVG(dom_complete)/1000) AS avg_page_load,
          -- CASE WHEN ROUND(AVG(dom_complete)/1000) >= 5 THEN TRUE ELSE FALSE END AS avg_over_four_sec
      FROM
          HEAP_T3_PLATFORM_PRODUCTION.HEAP.ANALYTICS_BROWSER_LOAD_APP AS e
  ...
  measure: avg_page_load_sec {
    ...
    link: {
      ...
      url: "/explore/heap_t3_platform/company_avg_page_load?fields=
      company_avg_page_load.duration_distribution,
      company_avg_page_load.total_users,
      company_avg_page_load.total_page_load_events
      &f[company_avg_page_load.date_in_period_date]={{ row['company_avg_page_load.date_in_period_date'] | url_encode }}
      &f[company_avg_page_load.current_date_range]={{ _filters['company_avg_page_load.current_date_range'] | url_encode }}
      &f[company_avg_page_load.device_type]={{ _filters['company_avg_page_load.device_type'] | url_encode }}
      &f[company_avg_page_load.browser_type]={{ _filters['company_avg_page_load.browser_type'] | url_encode }}
      &f[company_avg_page_load.region]={{ _filters['company_avg_page_load.region'] | url_encode }}
      &sorts=company_avg_page_load.duration_distribution"
    }

    link: {
      label: "Triage Top Dashboard Sections"
      url: "/explore/heap_t3_platform/company_avg_page_load?fields=
      company_avg_page_load.dashboard_section,
      company_avg_page_load.dashboard_name,
      company_avg_page_load.high_page_load_users,
      company_avg_page_load.slow_page_percentage,
      company_avg_page_load.p95_page_load_sec,
      company_avg_page_load.p95_page_load_sec,
      company_avg_page_load.p99_page_load_sec,
      company_avg_page_load.total_page_load_events
      &f[company_avg_page_load.date_in_period_date]={{ row['company_avg_page_load.date_in_period_date'] | url_encode }}
      &f[company_avg_page_load.current_date_range]={{ _filters['company_avg_page_load.current_date_range'] | url_encode }}
      &f[company_avg_page_load.device_type]={{ _filters['company_avg_page_load.device_type'] | url_encode }}
      &f[company_avg_page_load.browser_type]={{ _filters['company_avg_page_load.browser_type'] | url_encode }}
      &f[company_avg_page_load.region]={{ _filters['company_avg_page_load.region'] | url_encode }}
      &f[company_avg_page_load.page_load_greater_than_n]=>1
      &sorts=company_avg_page_load.high_page_load_users desc, company_avg_page_load.slow_page_percentage desc, company_avg_page_load.p95_page_load_sec desc, company_avg_page_load.total_page_load_events desc"
    }
  }
  ...
  measure: page_load_greater_than_n {
    ...
    sql: CASE
          WHEN (${TABLE}."DOM_COMPLETE" / 1000) >= 5 THEN 1
          ELSE NULL
        END ;;
    filters: [period_filtered_measures: "this"]
  }

  measure: prev_page_load_greater_than_n {
    type: sum
    label: "Count Page Load ≥ 5 sec"
    sql: CASE
          WHEN (${TABLE}."DOM_COMPLETE" / 1000) >= 5 THEN 1
  ...
  measure: prev_page_load_lower_than_n {
    type: sum
    label: "Count Page Load < 5 sec"
    sql: CASE
          WHEN (${TABLE}."DOM_COMPLETE" / 1000) < 5 THEN 1
  ...
  measure: high_page_load_users {
    ...
    sql: CASE
          WHEN (${TABLE}."DOM_COMPLETE" / 1000) >= 5 THEN ${user_id}
          ELSE NULL
        END ;;
    filters: [period_filtered_measures: "this"]
  ...
  measure: slow_page_percentage {
    type: number
    value_format_name: percent_2
    sql: (${company_avg_page_load.page_load_greater_than_n}) / ((${company_avg_page_load.page_load_greater_than_n}) + (${company_avg_page_load.page_load_lower_than_n})) ;;
  ...
  measure: prev_slow_page_percentage {
    type: number
    value_format_name: percent_2
    sql: (${company_avg_page_load.prev_page_load_greater_than_n}) / ((${company_avg_page_load.prev_page_load_greater_than_n}) + (${company_avg_page_load.prev_page_load_lower_than_n})) ;;
  ...
  measure: slow_percentage_pop_change {
    label: "Total percentage period-over-period Δ change"
    type: number
    sql: CASE WHEN ${slow_page_percentage} = 0
                THEN NULL
                ELSE (${slow_page_percentage} - NULLIF(${prev_slow_page_percentage} ,0)) END ;;
    value_format_name: percent_2
  }
