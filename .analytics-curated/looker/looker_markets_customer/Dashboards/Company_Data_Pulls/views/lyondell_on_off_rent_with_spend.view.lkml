#
# Copy of company_on_off_rent_with_spend.view, replace liquid variables with hardcoded company_id
# Replacing previous method of hardcoding the company_id in company_data_pull_data.model with a sql_always_where
# It's a hacky solution for now, but should be more performant
#
# Related Story:
#   [https://app.shortcut.com/businessanalytics/story/257851/data-pull-export-tool-add-deer-park-brittanie-mcinnis]
#
#
# Nick Logan | 6-11-2024

view: lyondell_on_off_rent_with_spend {
  derived_table: {
    sql: SELECT
          r.rental_id,
          o.order_id,
      ...
      where
          po.company_id = 107706
          AND c.company_id = 107706
          AND (
      ...
      where
        ...
        and u.company_id = 107706
        and po.company_id = 107706
        and (
        r.rental_status_id <> 5
        OR (r.rental_status_id <> 5 and r.asset_id is null)
        )
