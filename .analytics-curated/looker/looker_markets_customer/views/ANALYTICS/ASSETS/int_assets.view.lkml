view: int_assets {
  ...
  dimension_group: last_off_rent {
    type: time
    timeframes: [raw, time, date, week, month, quarter, year]
    sql: CAST(${TABLE}."LAST_OFF_RENT_DATE" AS TIMESTAMP_NTZ) ;;
  }
  ...
  dimension_group: last_rental_cleaned {
    type: time
    sql:
    CASE
      WHEN CAST(${TABLE}."LAST_RENTAL_DATE" AS DATE) = DATE '9999-12-31'
        THEN CURRENT_TIMESTAMP()
      ELSE CAST(${TABLE}."LAST_RENTAL_DATE" AS TIMESTAMP_NTZ)
    END ;;
  }
  dimension_group: latest_rental_start {
    type: time
    timeframes: [raw, time, date, week, month, quarter, year]
    sql: CAST(${TABLE}."LATEST_RENTAL_START_DATE" AS TIMESTAMP_NTZ) ;;
  }
  ...
  dimension: unavailable_flag {
    type: yesno
    sql: case when ${inventory_transit_status} in ('Make Ready', 'Needs Inspection', 'Soft Down','Hard Down') then true else false end ;;
  }
  measure: count {
    type: count
    drill_fields: [detail*]
  ...
  measure: unavailable_oec_percent {
    ...
    SUM(
      CASE
        WHEN ${inventory_transit_status} IN ('Make Ready', 'Needs Inspection', 'Soft Down', 'Hard Down')
        THEN ${TABLE}.oec
        ELSE 0
      END
    ) / NULLIF(SUM(${TABLE}.oec), 0) ;;
    value_format: "0.0%"
    drill_fields: [detail*]
  }
