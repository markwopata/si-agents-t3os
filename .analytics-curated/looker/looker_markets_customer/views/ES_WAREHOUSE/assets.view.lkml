view: assets {
  ...
  measure: total_assets_last_month {
    type: count_distinct
    sql: ${asset_id} ;;
    filters: [date_created_before_current_month: "Yes"]
  }

  measure: total_assets_last_year {
    type: count_distinct
    sql: ${asset_id} ;;
    filters: [date_created_before_current_year: "Yes"]
  }

  dimension: date_created_before_current_month {
    type: yesno
    sql: ${date_created_date} <= current_date - interval '1 month';;
  }

  dimension: date_created_before_current_year {
    type: yesno
    sql: date_part(year, ${date_created_raw}) < date_part(year, date_trunc('year', current_date)) ;;
  }
