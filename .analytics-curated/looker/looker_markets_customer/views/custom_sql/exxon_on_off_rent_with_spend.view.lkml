view: exxon_on_off_rent_with_spend {
  derived_table: {
    sql: SELECT
        r.rental_id,
        o.order_id,
    ...
    from
        ...
        select
            ...
            where
                c.company_id in (select company_id from es_warehouse.public.companies where name like '%Exxon%' and c.company_id <> 47396)
            group by
                r.rental_id, po.name
    ...
    where
        po.company_id in (select company_id from es_warehouse.public.companies where name like '%Exxon%' and c.company_id <> 47396)
        AND c.company_id in (select company_id from es_warehouse.public.companies where name like '%Exxon%' and c.company_id <> 47396)
        AND (
        r.rental_status_id = 5
        OR (r.rental_status_id = 5 AND r.asset_id is null)
        )
        and c.company_id <> 47396
    union
    select
        r.rental_id,
        o.order_id,
    ...
    where
      ...
      and u.company_id in (select company_id from es_warehouse.public.companies where name like '%Exxon%' and c.company_id <> 47396)
      and po.company_id in (select company_id from es_warehouse.public.companies where name like '%Exxon%' and c.company_id <> 47396)
      and (
      r.rental_status_id <> 5
      OR (r.rental_status_id <> 5 and r.asset_id is null)
      )
