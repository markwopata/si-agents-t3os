...
view: company_pulse_eom_oec_on_rent {
  derived_table: {
        ...
        , assigned_region as (
          ...
          where
            lower(work_email) = '{{ _user_attributes['email'] }}'
                  ...
                  where
                    date >= dateadd(month,-14,current_date)
                  ...
                  SELECT
                      ...
                      xw.region_name,
                      IFF(
                        IFF(rsc.total_regions_selected = 1, rs.region ,ar.region) = xw.region_name,TRUE,FALSE) as is_selected_region,
...
;;
  ...
  dimension: eliminate_dups {
    type: number
    sql: ${TABLE}."ELIMINATE_DUPS" ;;
  }
