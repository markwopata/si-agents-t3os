view: mlamd_region_highlighted {
  derived_table: {
    sql:
      ...
      , assigned_region as (
      select
      IFF(split_part(DEFAULT_COST_CENTERS_FULL_PATH,'/',2) = 'Corp', 'Midwest', IFF(split_part(DEFAULT_COST_CENTERS_FULL_PATH,'/',2) = 'R2 Mountain West', 'Mountain West',
      IFF(split_part(split_part(DEFAULT_COST_CENTERS_FULL_PATH,'/',2), ' ',2) = '','Midwest',split_part(split_part(DEFAULT_COST_CENTERS_FULL_PATH,'/',2), ' ',2)))) as region
      from
      analytics.payroll.company_directory
      where
      lower(work_email) = '{{ _user_attributes['email'] }}'
      group by  IFF(split_part(DEFAULT_COST_CENTERS_FULL_PATH,'/',2) = 'Corp', 'Midwest', IFF(split_part(DEFAULT_COST_CENTERS_FULL_PATH,'/',2) = 'R2 Mountain West', 'Mountain West',
      IFF(split_part(split_part(DEFAULT_COST_CENTERS_FULL_PATH,'/',2), ' ',2) = '','Midwest',split_part(split_part(DEFAULT_COST_CENTERS_FULL_PATH,'/',2), ' ',2))))
      )
    ...
    --              on vmt.market_id = mlamd.market_id
  ...
  measure: unavailable_oec_sum_selected {
    ...
    sql: ${unavailable_oec} ;;
    filters: [is_selected_region: "yes"]
    description: "Sum of oec of assets in rental fleet with inventory status of Make Ready, Needs Inspection, Soft Down, or Hard Down in selected region"
    value_format: "[>=1000000000]$0.00,,,\"B\";[>=1000000]$0.00,,\"M\";[>=1000]$0.00,\"K\";$0"
  }
  ...
  measure: unavailable_oec_sum_unselected {
    ...
    sql: ${unavailable_oec} ;;
    filters: [is_selected_region: "no"]
    description: "Sum of oec of assets in rental fleet with inventory status of Make Ready, Needs Inspection, Soft Down, or Hard Down NOT in selected region"
    value_format: "[>=1000000000]$0.00,,,\"B\";[>=1000000]$0.00,,\"M\";[>=1000]$0.00,\"K\";$0"
  }
