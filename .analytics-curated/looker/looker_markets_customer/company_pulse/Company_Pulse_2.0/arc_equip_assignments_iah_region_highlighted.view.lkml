view: arc_equip_assignments_iah_region_highlighted {
    derived_table: {
      sql:
      with
      region_selection as (
      ...
      where
      {% condition region_highlight_filter %} region_name {% endcondition %}
      ...
      ,split_part(split_part(DEFAULT_COST_CENTERS_FULL_PATH,'/',2), ' ',2)
        ...
        WHERE (xw.division_name <> 'Materials' OR xw.division_name IS NULL) and
        {% condition market_name_filter %} xw.market_name {% endcondition %}
  ...
  measure: actively_renting_customers_selected {
    ...
    description: "Count of customers in the selected region with rental assets on rent in a given time period."
    type: count_distinct
    sql: ${company_id} ;;
  ...
  measure: actively_renting_customers_unselected {
    ...
    label: "Actively Renting Customers"
    description: "Count of customers in unselected regions with rental assets on rent in a given time period."
    type: count_distinct
    sql: ${company_id} ;;
