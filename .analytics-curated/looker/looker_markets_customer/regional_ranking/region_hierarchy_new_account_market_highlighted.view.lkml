view: region_hierarchy_new_account_market_highlighted {
 derived_table: {
  sql:
    ...
    where (dates.is_current_month OR dates.is_prior_month) and
        ...
        {% condition district_name_filter %} xw.district {% endcondition %}
   )
...
measure: current_month_total_new_credit_app_accounts_selected {
  ...
  filters: [timeframe: "Current", app_type: "-COD", is_selected_market: "YES"]
  drill_fields: [customer_info*]
}
...
measure: current_month_total_new_credit_app_accounts_unselected {
  ...
  filters: [timeframe: "Current", app_type: "-COD", is_selected_market: "NO"]
  drill_fields: [customer_info*]
}
