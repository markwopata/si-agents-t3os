include: "/_base/analytics/payroll/recruiter_company_directory.view.lkml"

view: +recruiter_company_directory {
  label: "Recruiter Company Directory"
  ...
  dimension: period_over_period_position_effective_date {
    type: date
    group_label: "Period Over Period"
    sql:
      case
    when {{ select_period_over_period._parameter_value }} = 1 then date_trunc('MONTH',date(${position_effective_date}))
    when {{ select_period_over_period._parameter_value }} = 2 then date_trunc('WEEK',date(${position_effective_date}))
    when {{ select_period_over_period._parameter_value }} = 3 then date_trunc('QUARTER',date(${position_effective_date}))
    when {{ select_period_over_period._parameter_value }} = 4 then date_trunc('YEAR',date(${position_effective_date}))
    else date_trunc('MONTH',date(${position_effective_date}))
    end
    ;;
  }
  dimension: period_over_period_position_effective_flag {
    type: yesno
    group_label: "Period Over Period"
    sql:
      case
    when {{ select_period_over_period._parameter_value }} = 1 then ${position_effective_date_date} >= to_date(dateadd('month',-12,date_trunc('month',current_date)))
    when {{ select_period_over_period._parameter_value }} = 2 then ${position_effective_date_date} >= to_date(dateadd('day',-60,date_trunc('month',current_date)))
    when {{ select_period_over_period._parameter_value }} = 3 then ${position_effective_date_date} >= to_date(dateadd('month',-24,date_trunc('month',current_date)))
    when {{ select_period_over_period._parameter_value }} = 4 then ${position_effective_date_year} >= 2023
    when {{ select_period_over_period._parameter_value }} = 5 then ${position_effective_date_year} = 2024
    when {{ select_period_over_period._parameter_value }} = 6 then ${position_effective_date_year} = 2023
    else ${position_effective_date_date} >= to_date(dateadd('month',-12,date_trunc('month',current_date)))
    end
    ;;
  }
  dimension: period_over_period_position_effective_string {
    type: string
    group_label: "Period Over Period"
    sql:
      case
    when {{ select_period_over_period._parameter_value }} = 3 then CAST(EXTRACT(YEAR from DATE(${position_effective_date}))||' '||'Q'||EXTRACT(QUARTER from DATE(${position_effective_date})) as STRING)
    when {{ select_period_over_period._parameter_value }} in (1,5,6) then CAST(EXTRACT(YEAR from DATE(${position_effective_date}))||' '||MONTHNAME(DATE(${position_effective_date})) as STRING)
    when {{ select_period_over_period._parameter_value }} = 4 then CAST(EXTRACT(YEAR from DATE(${position_effective_date})) as STRING)
    else CAST(${period_over_period_position_effective_date} as STRING)
    end
    ;;
  }
