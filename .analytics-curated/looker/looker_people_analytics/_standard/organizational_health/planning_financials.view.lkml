view: planning_financials {
  derived_table: {
    sql:
    with df as ( select *, row_number() over (partition by MARKET_ID order by GL_DATE desc) as rn from analytics.branch_earnings.high_level_financials_snap WHERE NET_INCOME!=0),

            df2 as (SELECT MARKET_ID, sum(NET_INCOME) as NET_INCOME_3_MONTHS from df where rn=1 or rn=2 or rn=3 group by MARKET_ID),
