view: commission_statement_access {
  derived_table: {
    sql:
    (WITH RECURSIVE
      directory AS (
                       SELECT cd.first_name,
                              cd.last_name,
                              cd.employee_title,
                              cd.employee_id,
                              IFF(cd.employee_id = 103, NULL, cd.direct_manager_employee_id) AS manager_id,
                              m.work_email                                                   AS manager_email,
                              cd.location
                         FROM analytics.payroll.company_directory cd
                                  INNER JOIN analytics.payroll.company_directory m
                                  ON cd.direct_manager_employee_id = m.employee_id
                        WHERE cd.date_terminated IS NULL),
      managers  AS (
                       SELECT ''                                    AS manager_list,
                              CONCAT_WS(' ', first_name, last_name) AS name,
                              employee_title,
                              employee_id,
                              manager_id,
                              manager_email,
                              location
                         FROM directory
                        WHERE employee_id = 103
                        UNION ALL
                       SELECT CONCAT_WS(',', emp.manager_email, man.manager_list) AS manager_list,
                              CONCAT_WS(' ', emp.first_name, emp.last_name)       AS name,
                              emp.employee_title,
                              emp.employee_id,
                              emp.manager_id,
                              emp.manager_email,
                              emp.location
                         FROM directory emp
                                  INNER JOIN managers man
                                  ON emp.manager_id = man.employee_id),
--one time override requested by Mark W on 5/19/23, completed by JW 5/23/23
--second override (David) requested by Mark W on 6/28/23, completed by JW 6/28/23
      overrides AS (
                       SELECT CASE WHEN CONTAINS(manager_list, 'eric@equipmentshare.com')
                                       THEN REPLACE(manager_list, 'eric@equipmentshare.com', 'eric@equipmentshare.com,jay.mitchell@equipmentshare.com,david.ross@equipmentshare.com')
                                   WHEN CONTAINS(manager_list, 'justin.ingold@equipmentshare.com') AND CONTAINS(location, 'Advanced Solutions')
                                       THEN REPLACE(manager_list, 'justin.ingold@equipmentshare.com', 'justin.ingold@equipmentshare.com,victor.otalora@equipmentshare.com')
                                   WHEN CONTAINS(manager_list, 'josh.cook@equipmentshare.com')
                                       THEN REPLACE(manager_list, 'josh.cook@equipmentshare.com', 'josh.cook@equipmentshare.com,ronny.robinson@equipmentshare.com')
                                  WHEN CONTAINS(manager_list, 'anthony.hobaica@equipmentshare.com')
                                       THEN REPLACE(manager_list, 'anthony.hobaica@equipmentshare.com', 'anthony.hobaica@equipmentshare.com,ronny.robinson@equipmentshare.com')
                                  ELSE manager_list END AS manager_list,
                              name,
                              employee_title,
                              employee_id,
                              manager_id,
                              manager_email
                         FROM managers)
SELECT STRTOK_TO_ARRAY(manager_list, ',') AS manager_array,
       m.name,
       m.employee_title,
       m.employee_id,
       u.user_id,
       m.manager_id
  FROM overrides m
           INNER JOIN es_warehouse.public.users u
           ON m.employee_id = TRY_TO_NUMBER(u.employee_id)
           INNER JOIN analytics.public.commissions_salesperson_data csd
           ON csd.salesperson_user_id = u.user_id
 WHERE (CURRENT_DATE BETWEEN guarantee_start_date AND commission_end_date AND u.company_id = 1854))

 UNION

 (WITH RECURSIVE
      directory AS (
                       SELECT cd.first_name,
                              cd.last_name,
                              cd.employee_title,
                              cd.employee_id,
                              IFF(cd.employee_id = 103, NULL, cd.direct_manager_employee_id) AS manager_id,
                              m.work_email                                                   AS manager_email,
                              cd.location
                         FROM analytics.payroll.company_directory cd
                                  INNER JOIN analytics.payroll.company_directory m
                                  ON cd.direct_manager_employee_id = m.employee_id
                        WHERE cd.date_terminated IS NULL),
      managers  AS (
                       SELECT ''                                    AS manager_list,
                              CONCAT_WS(' ', first_name, last_name) AS name,
                              employee_title,
                              employee_id,
                              manager_id,
                              manager_email,
                              location
                         FROM directory
                        WHERE employee_id = 103
                        UNION ALL
                       SELECT CONCAT_WS(',', emp.manager_email, man.manager_list) AS manager_list,
                              CONCAT_WS(' ', emp.first_name, emp.last_name)       AS name,
                              emp.employee_title,
                              emp.employee_id,
                              emp.manager_id,
                              emp.manager_email,
                              emp.location
                         FROM directory emp
                                  INNER JOIN managers man
                                  ON emp.manager_id = man.employee_id),
--one time override requested by Mark W on 5/19/23, completed by JW 5/23/23
--second override (David) requested by Mark W on 6/28/23, completed by JW 6/28/23
      overrides AS (
                       SELECT CASE WHEN CONTAINS(manager_list, 'eric@equipmentshare.com')
                                       THEN REPLACE(manager_list, 'eric@equipmentshare.com', 'eric@equipmentshare.com,jay.mitchell@equipmentshare.com,david.ross@equipmentshare.com')
                                   WHEN CONTAINS(manager_list, 'justin.ingold@equipmentshare.com') AND CONTAINS(location, 'Advanced Solutions')
                                       THEN REPLACE(manager_list, 'justin.ingold@equipmentshare.com', 'justin.ingold@equipmentshare.com,victor.otalora@equipmentshare.com')
                                   WHEN CONTAINS(manager_list, 'josh.cook@equipmentshare.com')
                                       THEN REPLACE(manager_list, 'josh.cook@equipmentshare.com', 'josh.cook@equipmentshare.com,ronny.robinson@equipmentshare.com')
                                  WHEN CONTAINS(manager_list, 'anthony.hobaica@equipmentshare.com')
                                       THEN REPLACE(manager_list, 'anthony.hobaica@equipmentshare.com', 'anthony.hobaica@equipmentshare.com,ronny.robinson@equipmentshare.com')
                                  ELSE manager_list END AS manager_list,
                              name,
                              employee_title,
                              employee_id,
                              manager_id,
                              manager_email
                         FROM managers)
SELECT STRTOK_TO_ARRAY(manager_list, ',') AS manager_array,
       m.name,
       m.employee_title,
       m.employee_id,
       u.user_id,
       m.manager_id
  FROM overrides m
           INNER JOIN es_warehouse.public.users u
           ON m.employee_id = TRY_TO_NUMBER(u.employee_id)
where m.employee_title ilike 'rental coordinator');;
  }

  dimension: name {
    label: "Employee Name"
    type: string
    sql: ${TABLE}."NAME" ;;
  }

  dimension: employee_title {
    type: string
    sql: ${TABLE}."EMPLOYEE_TITLE" ;;
  }

  dimension: employee_id {
    type: number
    sql: ${TABLE}."EMPLOYEE_ID" ;;
    value_format_name: id
  }

  dimension: user_id {
    label: "Employee User ID"
    type: number
    sql: ${TABLE}."USER_ID" ;;
    value_format_name: id
  }

  dimension: manager_id {
    label: "Direct Manager Employee ID"
    type: number
    sql: ${TABLE}."MANAGER_ID";;
    value_format_name: id
  }

  dimension: manager_array {
    label: "Manager List"
    type: string
    sql: ${TABLE}."MANAGER_ARRAY"::VARIANT ;;
  }
}
