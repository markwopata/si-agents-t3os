...
include: "/_standard/custom_sql/high_level_financials.view"
...
include: "/_standard/analytics/payroll/company_directory_dsm.layer.lkml"


explore: v_dim_candidate {
  label: "Scorecard Effectiveness"
  ...
  join: company_directory {
    type: left_outer
    relationship: one_to_one
    sql_on: ${company_directory.greenhouse_application_id} = ${v_dim_application.application_id}  ;;
  }
  ...
  join: high_level_financials {
    type: left_outer
    relationship: one_to_one
    sql_on: ${company_directory.market_id}= ${high_level_financials.mkt_id};;
  }
...
explore: invoices {
  label: "DSM Scorecard Performance"
  ...
  join: employee_disc_codes {
    type: left_outer
    relationship: one_to_one
    sql_on: ${company_directory_dsm.employee_id} = ${employee_disc_codes.employee_id} ;;
  }

  join: v_fact_application_requisition_offer {
    type: left_outer
    relationship: one_to_one
    sql_on: ${company_directory_dsm.greenhouse_application_id} = ${v_fact_application_requisition_offer.application_requisition_offer_application_key} ;;
  }
  ...
  join: company_directory_dsm {
    type: inner
    relationship: many_to_one
    sql_on: try_to_number(${company_directory.direct_manager_employee_id}) = try_to_number(${company_directory_dsm.employee_id});;
  }
