  ...
  derived_table: {
    sql:   WITH guarantee AS (SELECT csd.salesperson_user_id,
                          ...
                          CASE
                              ...
                              WHEN date_trunc('month',csd.GUARANTEE_START_DATE) = DATEADD('month', 1, {% parameter commission_month_filter %}) and
                                   DAY(csd.guarantee_start_date) != 1
                                  then current_month_days - day(csd.GUARANTEE_START_DATE)
...
FROM guarantee g
  ...
  dimension: commission_type {
    description: "Commission plan based on employee title and/or market assignment."
    type: string
  ...
  measure: total_guarantee {
    type: sum
    sql: coalesce(${prorated_guarantee},0)+coalesce(${current_guarantee},0) ;;
    value_format_name: usd
  }
