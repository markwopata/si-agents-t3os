...
target_core_mkt as (select acctno,
                                 GL_ACCT,
                                 type,
                                 round(sum(amt) / 96, 2)  as target_amount,
                                 target_amount / 41400023 as pct_oec_target,
                                 'Core Solutions' as market_type
                          from ANALYTICS.PUBLIC.BRANCH_EARNINGS_DDS_SNAP
                          where date_trunc(month, GL_DATE) >= '2024-06-01'
                            and date_trunc(month, GL_DATE) <= '2025-05-01'
                            and mkt_id in (18702, 63125, 24007,  24079, 35789, 15984, 16835, 40682)
                          group by all
),
all_targets as (
...
where at.type not in ('Intercompany Transactions','Sales Revenues','Cost of Sales Revenues');;
  ...
  dimension: gl_acct {
    ...
    link: {
      label: "Detail View"
      url: "@{lk_be_transaction_detail}?&f[market_region_xwalk.market_name]={{ _filters['target_market_comparison.market_name']}}&f[plexi_periods.display]={{ _filters['plexi_periods.display']}}&f[be_transaction_listing.gl_acctno]={{ target_market_comparison.acctno._value}}&toggle=fil"
    }
  }
  ...
  dimension: type {
    ...
    link: {
      label: "Detail View"
      url: "@{lk_be_bucket_detail}?&f[market_region_xwalk.market_name]={{ _filters['target_market_comparison.market_name']}}&f[plexi_periods.display]={{ _filters['plexi_periods.display']}}&f[be_transaction_listing.type]={{ target_market_comparison.type._value}}&toggle=fil"
    }
  }
