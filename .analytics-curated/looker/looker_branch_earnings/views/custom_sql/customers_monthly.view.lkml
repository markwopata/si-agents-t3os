view: customers_monthly {
  derived_table: {
    sql:
     with customer_monthly as (select mrx.market_id
      ...
      coalesce(datediff(month,m.BRANCH_EARNINGS_START_MONTH,tc.billing_month) +1,0) as months_open,
      tc.district,tc.region, tc.MARKET_TYPE
  ...
  dimension: customer_rental_share {
    label: "Customer Rental Share"
    type: number
    sql: ${TABLE}."TC_RENTAL_SHARE" ;;
  }
  dimension: customer_revenue_share {
