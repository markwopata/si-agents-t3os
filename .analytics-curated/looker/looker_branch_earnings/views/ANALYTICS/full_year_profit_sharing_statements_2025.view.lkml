view: full_year_profit_sharing_statements_2025 {
  derived_table: {
    sql: with total_by_ee as (
          select ID,
                 sum(PROFIT_SHARE_MULTIPLIER_AMOUNT) as ps_ttl_by_ee,
          from ANALYTICS.BRANCH_EARNINGS.FULL_YEAR_PROFIT_SHARING_STATEMENTS_2025
          group by ID
          )
          select fy.*, ttl.ps_ttl_by_ee
          from ANALYTICS.BRANCH_EARNINGS.FULL_YEAR_PROFIT_SHARING_STATEMENTS_2025 fy
  ...
  measure: bonus_pool_portion_profit_share_percent_sum {
    ...
    type: max
    value_format: "#,##0%;(#,##0%);-"
    sql: case when ${TABLE}."BONUS_POOL_PORTION" <= 0 then 0 else ${bonus_pool_portion_profit_share_percent} end;;
  }
  ...
  measure: branch_greater_12_months_prorated_percent {
    ...
    type: average
    value_format: "#,##0%;(#,##0%);-"
    sql: case when ${TABLE}."BONUS_POOL" = 0 then 0 else ${branch_greater_12_months_prorated} end ;;
  }
  ...
  measure: employment_duration_eligibility_percent_sum {
    ...
    type: average
    value_format: "#,##0%;(#,##0%);-"
    sql: case when ${TABLE}."BONUS_POOL_PORTION" = 0 then 0 else ${employment_duration_eligibility_percent} end ;;
  }
  ...
  dimension: id_name_statement_link {
    ...
    link: {
      label: "Statement"
      url: "@{db_full_year_profit_sharing_statement_2025}?ID+Name={{ full_year_profit_sharing_statements_2025.id_name | url_encode }}&Profit+Share+Period={{ full_year_profit_sharing_statements_2025.quarter_timestamp | url_encode }}"
    }
    sql:  ${TABLE}."ID_NAME" ;;
  }
  ...
  dimension: cost_center {
    ...
    sql: case when ${market} is null and ${district} is null then ${region}
              ...
              when ${market} is not null then concat(${market},' - D',${district},' - ', ${region}) else null end;;
  }
