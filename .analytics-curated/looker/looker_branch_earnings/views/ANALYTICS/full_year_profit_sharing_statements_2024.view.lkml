view: full_year_profit_sharing_statements_2024 {
  derived_table: {
    sql: with total_by_ee as (
          select ID,
                 sum(PROFIT_SHARE_MULTIPLIER_AMOUNT) as ps_ttl_by_ee,
          from ANALYTICS.BRANCH_EARNINGS.FULL_YEAR_PROFIT_SHARING_STATEMENTS_2024
          group by ID
          )
          select fy.*, ttl.ps_ttl_by_ee
          from ANALYTICS.BRANCH_EARNINGS.FULL_YEAR_PROFIT_SHARING_STATEMENTS_2024 fy
  ...
  dimension: id_name_statement_link {
    ...
    link: {
      label: "Statement"
      url: "@{db_full_year_profit_sharing_statement_2024}?ID+Name={{ full_year_profit_sharing_statements_2024.id_name | url_encode }}&Profit+Share+Period={{ full_year_profit_sharing_statements_2024.quarter_timestamp | url_encode }}"
    }
    sql:  ${TABLE}."ID_NAME" ;;
  }
