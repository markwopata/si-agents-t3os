  view: wos_within_7days_of_delivery_aggregate {
    sql_table_name: "ANALYTICS"."SERVICE"."SERVICE_BRANCH_SCORECARD_7_DAY_BREAKDOWN"
    ;;
    # derived_table: {
    #   sql:
    #   --all deliveries
    ...
    #     --        d.DELIVERY_ID,
    #     --        d.RENTAL_ID,
    #     CASE WHEN wos_7.WORK_ORDER_ID is null then d.BRANCH_ID ELSE COALESCE(wos_last.BRANCH_ID, wos_7.BRANCH_ID, d.BRANCH_ID) end as BRANCH_ID,
    #     DATE_TRUNC('month', COALESCE(wos_7.WO_DATE,d.DELIVERY_DATE))::DATE as month,
    #     CASE WHEN wos_7.WORK_ORDER_ID is not null then 1 else 0 end as breakdown_7_day,
    #     1 as delivery
    #     --        wos_7.WORK_ORDER_ID,
    #     --        wos_7.WORK_ORDER_TYPE_NAME,
    #     --        wos_7.WORK_ORDER_STATUS_NAME,
    ...
    #     --        d.RENTAL_START,
    #     --        d.RENTAL_END,
    #     --        ROW_NUMBER() OVER(PARTITION BY d.ASSET_ID, wos_7.WORK_ORDER_ID ORDER BY wos_last.WO_DATE desc)
    #     from deliveries d
    #     left join wos as wos_7-- to get breakdowns within 7 days
    #     on d.ASSET_ID = wos_7.asset_id
    #     and wos_7.wo_date between d.delivery_date and dateadd('days', 7, d.delivery_date)
    ...
    #     QUALIFY ROW_NUMBER() OVER(PARTITION BY d.ASSET_ID, wos_7.WORK_ORDER_ID ORDER BY wos_last.WO_DATE desc) = 1 -- qualifier to get the most recent wo before delivery
    #     )
    ...
    #     branch_id,
    #     month,
    #     sum(breakdown_7_day) as count_wos_within_7_days_delivery,
    #     sum(delivery) as delivery_count,
    #     sum(breakdown_7_day) / sum(delivery) as percent_deliveries_with_wo_within_7_days,
    #     COALESCE(LEAST(1-(sum(breakdown_7_day) / sum(delivery)),1),1) as percent_to_goal,
    ...
    dimension: count_wos_within_7_days_delivery {
      type: number
      value_format: "#,##0"
      sql: COALESCE(${TABLE}.count_wos_within_7_days_delivery,0) ;;
    }
    ...
    dimension: percent_deliveries_with_wo_within_7_days {
      type: number
      value_format: "0.0%"
      sql: COALESCE(${TABLE}.percent_deliveries_with_wo_within_7_days,0) ;;
    }
    ...
    measure:  drill_fields_sbs{
      ...
      drill_fields: [
        ...
        wos_within_7days_of_delivery_aggregate.count_wos_within_7_days_delivery,
        wos_within_7days_of_delivery_aggregate.delivery_count,
        wos_within_7days_of_delivery_aggregate.percent_deliveries_with_wo_within_7_days,
        wos_within_7days_of_delivery_aggregate.percent_to_goal,
        wos_within_7days_of_delivery_aggregate.score]
    }
    ...
    measure: avg_last_1_month {
      ...
      link: {
        label: "Service Tech Scorecard Dashboard"
        url: "https://equipmentshare.looker.com/dashboards-next/847?Market+Name={{ _filters['service_branch_scorecard.market_name'] | url_encode }}&toggle=det"
      }
    }
    ...
    measure: avg_last_3_months_performance {
      ...
      value_format: "#,##0"
      sql: CASE
                   WHEN ${service_branch_scorecard.is_last_3_months} THEN ${count_wos_within_7_days_delivery}
                   ELSE NULL
                 END ;;
      description: "Average of monthly metric for the past 3 months"
      html:
