...
view: accumulated_depreciation_in_current_status {
  derived_table: {
    sql:
  select scd.asset_id
    , COALESCE(scd.ASSET_INVENTORY_STATUS, 'Unassigned') as asset_status
    , DATEDIFF('day', date_start, current_date()) days_in_status
    , (days_in_status * ((0.015 / (365 / 12)) * aa.oec)) as accumulated_depreciation
  from ${scd_asset_inventory_status.SQL_TABLE_NAME} scd
  join ES_WAREHOUSE.PUBLIC.ASSETS_AGGREGATE aa
    on aa.asset_id = scd.asset_id
...
;;
  ...
  dimension: accumulated_depreciation_in_current_status {
    type: number
