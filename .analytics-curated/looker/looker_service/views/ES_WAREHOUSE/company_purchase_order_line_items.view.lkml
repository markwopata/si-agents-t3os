view: company_purchase_order_line_items {
  ...
  dimension: company_purchase_order_line_item_id {
    primary_key: yes
    type: number
    sql: CAST(${TABLE}."COMPANY_PURCHASE_ORDER_LINE_ITEM_ID" as VARCHAR) ;;
  }
  ...
  measure: count {
    type: count
    drill_fields: [company_purchase_order_line_item_id, company_purchase_orders.company_purchase_order_id]
  }
  dimension: licensed {
    type: yesno
    sql: ifnull(${license_state_id},-1) >0  ;;
  }
}
