view: part_substitutes {
  ...
  dimension: part_type {
    type: string
    sql: iff(${v_parts.part_id} = ${sub_part_id},'REMAN','NEW') ;;
  }
  dimension: reman_part_number {
    type: string
    sql: iff(${part_type} = 'NEW',${sub_part_number},null) ;;
  }
  dimension: new_part_number {
    type: string
    sql: iff(${part_type} = 'REMAN',${part_number},null) ;;
  }
  measure: sub_type_list {
    type: string
    sql: LISTAGG(DISTINCT concat(${substitution_type}||'-'||${isactive}), ', ') over (partition by ${part_id}) ;;
  }

  measure: has_reman_sub {
    type: yesno
    sql: ${sub_type_list} ilike '%REMAN-true%' ;;
  }

  measure: has_aftermarket_sub {
    type: yesno
    sql: ${sub_type_list} ilike '%Aftermarket Vendor-true%' ;;
  }
}
