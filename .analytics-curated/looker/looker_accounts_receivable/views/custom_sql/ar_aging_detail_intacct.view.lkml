view: ar_aging_detail_intacct {
  derived_table: {
    sql: SELECT
          ...
          COALESCE(ARH.UNDEPOSITEDACCOUNTNO, ARH.FINANCIALENTITY)                           AS PAY_SOURCE,
          ...
          COALESCE(ARP_INV.AMT * -1, 0) + CASE WHEN ARH.RECORDTYPE = 'aradvance' THEN ARD.AMOUNT ELSE 0 END AS NET_PAYMENT,
          ...
          CASE WHEN COUNT(CASE WHEN (ORIG_AMOUNT + AMT_PAID + AMT_CM_APPLIED + PAYMENT_APPLIED) > 0 THEN 1 ELSE NULL END)
                   OVER (PARTITION BY ARH.CUSTOMERID) >= 1 THEN '-' ELSE 'Yes' END          AS ESCHEATMENT
      FROM
          ANALYTICS.INTACCT.ARRECORD ARH
              ...
              LEFT JOIN (SELECT
                            ...
                            SUM(CASE WHEN ARR.UNDEPOSITEDACCOUNTNO IN ('5316', '1205') THEN ARIP.AMOUNT ELSE 0 END)     AS AMT_PAY_APPLIED_WO,
                        ...
                        GROUP BY ALL) AS ARDET1 ON ARH.RECORDNO = ARDET1.RECORDKEY
                        ...
                        FROM
                            ANALYTICS.GS.COLLECTOR_CUSTOMER_ASSIGNMENTS CCA
                                LEFT JOIN ANALYTICS.GS.COLLECTOR_OIL_GAS_LEGAL COGL
                                          ON CCA.COMPANY_ID = COGL.CUSTOMER_ID and COGL.MONTH_RETURNED_FROM_LEGAL is null) COLLECTOR
              ...
              LEFT JOIN(SELECT DISTINCT
                        ...
                        FROM
                            ES_WAREHOUSE.PUBLIC.COMPANIES ADM_CUST
                                ...
                                LEFT JOIN ES_WAREHOUSE.PUBLIC.STATES ST ON LOC.STATE_ID = ST.STATE_ID) CUST_ADDRESS ON ARH.CUSTOMERID = TO_CHAR(CUST_ADDRESS.CUSTOMER_ID)
      WHERE
            ((ARH.WHENPOSTED <= {% date_end date_filter %} AND ARH.RECORDTYPE != 'aradvance') OR (ARH.WHENPAID <= {% date_end date_filter %} AND ARH.RECORDTYPE = 'aradvance'))
  ...
  dimension: pay_source {
    type: string
    sql: ${TABLE}."PAY_SOURCE" ;;
  }
