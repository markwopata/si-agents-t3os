view: utilization_daily_summary {
  derived_table: {
    sql:
    with bdu_f AS (
  SELECT *
  ...
  dimension: primary_key {
    primary_key: yes
    type: string
    sql: concat(${day},${rented_vs_owned},${total_on_time},${total_available_assets},${date_filter_difference} ) ;;
  }
  ...
  dimension: rented_vs_owned {
    label: "Ownership"
    type: string
    sql: ${TABLE}."RENTED_VS_OWNED" ;;
  }
  ...
  dimension: complete_week_formatted {
    type: date
    group_label: "HTML Date Week"
    label: "Complete Week"
    sql: ${day_grouping_week} ;;
    html: Week of {{ rendered_value | date: "%b %d, %Y" }};;
  }
  ...
  measure: total_selected_on_time {
    label: "Total On Time"
    type: sum
    sql: coalesce(round(${total_on_time},0),0) ;;
  ...
  measure: dynamic_average_utilization_percentage_by_day {
    ...
    html: {{rendered_value}}
          ...
          <br />Total On Time <br />{{total_selected_on_time._rendered_value}} hrs.;;
    type: number
    value_format_name: percent_1
  }
  ...
  dimension: is_current_week {
    ...
    sql:
    CASE
      WHEN DATE_TRUNC('WEEK', ${TABLE}."DAY") = DATE_TRUNC('WEEK', CURRENT_DATE)
      THEN TRUE
      ELSE FALSE
    END ;;
  }

  measure: current_calendar_week_selected_on_time {
    type: sum
    sql: coalesce(round(${total_on_time},0),0) ;;
    filters: [is_current_week: "Yes"]
  }

  measure: current_calendar_week_available_assets {
    type: sum
    sql: ${total_available_assets} ;;
    filters: [is_current_week: "Yes"]
  }

  measure: daily_utilization_kpi_eight_hours_current_week {
    type: number
    sql:
    ((${current_calendar_week_selected_on_time})/(8))/(case when (${current_calendar_week_available_assets}) = 0 then null else (${current_calendar_week_available_assets}) end)
    ;;
    value_format_name: percent_1
  }
