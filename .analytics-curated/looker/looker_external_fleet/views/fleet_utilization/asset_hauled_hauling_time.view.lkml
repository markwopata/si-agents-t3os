view: asset_hauled_hauling_time {
  derived_table: {
    sql:
     with bdu_f AS (
          SELECT *
    ...
    select distinct
          ...
          /* , case
          when '{{ _user_attributes['user_timezone'] }}' = 'UTC' then hauled_time_utc
          when '{{ _user_attributes['user_timezone'] }}' = 'America/Chicago' then hauled_time_cst
          when '{{ _user_attributes['user_timezone'] }}' = 'America/Denver' then hauled_time_mnt
          when '{{ _user_attributes['user_timezone'] }}' = 'America/Los_Angeles' then hauled_time_wst
          --- else is Eastern Standard Time
          else hauled_time_est
          end as hauled_time
          , case
          when '{{ _user_attributes['user_timezone'] }}' = 'UTC' then hauling_time_utc
          when '{{ _user_attributes['user_timezone'] }}' = 'America/Chicago' then hauling_time_cst
          when '{{ _user_attributes['user_timezone'] }}' = 'America/Denver' then hauling_time_mnt
          when '{{ _user_attributes['user_timezone'] }}' = 'America/Los_Angeles' then hauling_time_wst
          --- else is Eastern Standard Time
          else hauling_time_est
          end as hauling_time
          */
           , case
