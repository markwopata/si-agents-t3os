  ...
  derived_table: {
    sql: with asset_list_own as (
      ...
      select
        alo.asset_id,
        case
        when convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', {% date_end hourly_asset_usage_date_filter.date_filter %})::date >= current_date then coalesce(al.geofences,al.address,al.location)
        else coalesce(hal.geofences,hal.address,hal.location) end as location,
        case
        when convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', {% date_end hourly_asset_usage_date_filter.date_filter %})::date >= current_date then al.last_checkin_timestamp
        else hal.last_checkin_timestamp end as last_checkin_timestamp
      from
        asset_list_own alo
        left join es_warehouse.snapshot.asset_last_location hal on alo.asset_id = hal.asset_id and convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', {% date_end hourly_asset_usage_date_filter.date_filter %})::date = hal.end_date::date
