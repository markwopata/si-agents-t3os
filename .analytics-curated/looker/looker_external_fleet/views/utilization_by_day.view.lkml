...
--      ),
      ...
      from
          ...
          join trackers_mapping tm on tm.asset_id = al.asset_id
      where
          ...
          AND (tm.tracker_tracker_id is not null OR tm.esdb_tracker_id is not null)
      group by
          al.asset_id,
          al.ownership_type,
      ...
      from
          ...
          join trackers_mapping tm on tm.asset_id = alr.asset_id
      where
          report_range:start_range >= COALESCE(convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC',{% date_start hourly_asset_usage_date_filter.date_filter %}),convert_timezone('{{ _user_attributes['user_timezone'] }}', 'UTC', current_date - interval '10 days'))
          AND report_range:end_range <= COALESCE(convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', {% date_end hourly_asset_usage_date_filter.date_filter %}),convert_timezone('{{ _user_attributes['user_timezone'] }}', 'UTC', current_date))
          AND {% condition asset_names_filter %} at.name {% endcondition %}
          AND {% condition custom_name_filter %} a.custom_name {% endcondition %}
          --AND {% condition groups_filter %} org.name {% endcondition %}
          AND {% condition category_filter %} cat.name {% endcondition %}
      ...
      group by
       ...
       where
          ...
          AND {% condition used_unused_filter %} a.asset_type {% endcondition %}
          AND {% condition asset_names_filter %} at.name {% endcondition %}
          AND {% condition custom_name_filter %} a.custom_name {% endcondition %}
          --AND {% condition groups_filter %} org.name {% endcondition %}
          AND {% condition category_filter %} cat.name {% endcondition %}
       ...
       from
          ...
          join trackers_mapping tm on tm.asset_id = alo.asset_id
       where
