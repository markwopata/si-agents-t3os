...
where report_range:start_range >= to_varchar({% date_start hourly_asset_usage_date_filter.date_filter %}, 'yyyy-mm-dd hh:mm:ss TZH')
union
select f.value as trip_id
from
...
select
        ...
        t.trip_id,
        case
            when t.start_timestamp < {% date_start hourly_asset_usage_date_filter.date_filter %} then {% date_start hourly_asset_usage_date_filter.date_filter %}
            else t.start_timestamp
        end start_timestamp,
        case
            when t.end_timestamp > {% date_end hourly_asset_usage_date_filter.date_filter %} then {% date_end hourly_asset_usage_date_filter.date_filter %}
        ...
        case
            when t.start_timestamp < {% date_start hourly_asset_usage_date_filter.date_filter %} then round((t.trip_time_seconds - datediff(second, t.start_timestamp, {% date_start hourly_asset_usage_date_filter.date_filter %}))/3600,2)
            when t.end_timestamp > {% date_end hourly_asset_usage_date_filter.date_filter %} then round((t.trip_time_seconds - datediff(second, {% date_end hourly_asset_usage_date_filter.date_filter %}::timestamp_ntz, t.end_timestamp))/3600,2)
     ...
     from
        ...
        and
        t.end_timestamp >= {% date_start hourly_asset_usage_date_filter.date_filter %}
                   and t.start_timestamp <= {% date_end hourly_asset_usage_date_filter.date_filter %}
  ...
  measure: total_idle_hours {
    ...
    sql: ${idle_hours} ;;
    value_format_name: decimal_2
    html: <font color="#0063f3"><u><a href="https://app.estrack.com/#/assets/all/asset/{{ trip_details.asset_id._value }}/history?selectedDate={{ trip_details.start_timestamp_date._value }}" target="_blank">{{ value }}</a></font></u> ;;
  }
  ...
  dimension: idle_minutes_show_link_or_zero {
    ...
    html:
    {% if value > 0 %}
    <p style="color: blue; font-weight: bold"><font color="#0063f3"><u><a href="https://app.estrack.com/#/assets/all/asset/{{ trip_details.asset_id._value }}/history?selectedDate={{ trip_details.start_timestamp_date._value }}" target="_blank">{{ idle_minutes_show_link_or_zero._value }}</a></font></u></p>
    {% else %}
