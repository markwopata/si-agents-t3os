view: historical_revenue_drill {
 derived_table: {
  sql: select
          concat(usales.first_name,' ',usales.last_name) as sales_rep,
          convert_timezone('{{ _user_attributes['user_timezone'] }}',li.created_date) as created_month,
          usales.user_id,
          m.name as branch,
          i.invoice_no,
          c.name as company,
          sum(li.total) as rental_revenue
      from
      ...
      group by
          ...
          convert_timezone('{{ _user_attributes['user_timezone'] }}',li.created_date),
          usales.user_id,
          m.name,
          c.name,
          i.invoice_no
  ...
  dimension: primary_key {
    primary_key: yes
    type: string
    sql: concat(${sales_rep},${user_id},${invoice_no},${created_month_raw}) ;;
  }
  ...
  dimension: formatted_created_date {
  ...
  set: detail {
    fields: [sales_rep, created_month_time, user_id, branch, billed_amount]
  }
}
