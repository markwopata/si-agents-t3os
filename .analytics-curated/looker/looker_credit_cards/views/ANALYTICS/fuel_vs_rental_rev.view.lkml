    ...
    sql:WITH month_series AS (
          ...
          fuel_transactions AS (
              ...
              WHERE transaction_date >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 YEAR')
                AND transaction_card_type = 'fuel'
          ...
          transactions_revenue_combined AS (
              SELECT
                  ...
                  ft.verified_transactions,
                  ft.unverified_transactions--,
                  --cc.reallocated_transactions,
                  --cc.travel_platform_transactions
              FROM rental_revenues rr
              FULL OUTER JOIN fuel_transactions ft
              ON rr.employee_id = ft.employee_id AND rr.month = ft.month
          )
      ...
      ZEROIFNULL(trc.total_transactions) as total_transactions,
