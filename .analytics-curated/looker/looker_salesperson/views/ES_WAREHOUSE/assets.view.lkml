view: assets {
  ...
  dimension: rerent {
    type: yesno
    sql: case
            when (SUBSTR(TRIM(${serial_number}), 1, 3) = 'RR-' and SUBSTR(TRIM(${serial_number}), 1, 2) != 'RR') then 'Yes'
            when (SUBSTR(TRIM(${custom_name}), 1, 3) = 'RR-' and SUBSTR(TRIM(${custom_name}), 1, 2) != 'RR') then 'Yes'
            when ${company_id} = 11606 then 'Yes'
            else 'No' end;;
  }
  ...
  measure: avg_age_of_assets {
    type: average
    sql:  date_part(year,(date_trunc('year', current_date))) - ${year};;
