view: companies_revenue_last_30_days {
  derived_table:{
    sql:
    select
            c.company_id,
...
left join ES_WAREHOUSE.public.companies c
  on cu.company_id=c.company_id
            where
              ...
              and date_trunc('month',li.gl_date_created ::DATE) >= (date_trunc('month',current_date) - interval '30 days')
            group by
              c.company_id,
            c.name
    ;;
  ...
  dimension: total_rev {
    ...
    type: number
    sql: coalesce(${TABLE}."TOTAL_REV", 0);;
