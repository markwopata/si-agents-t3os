view: company_prospect_lookup_v2 {
  derived_table: {
    sql:  with invoices_info as (
      ...
      where
        c.timezone not like '%Auckland%')
         select u.company_id::varchar(15000) as company_prospect_id, ex.company_name as company_name, '' as sales_representative_email_address,
...
left join ANALYTICS.prospects.existing_companies_mapping as ex
...
union all
        select pm.prospect_id as company_prospect_id , pm.company_name as company_name , pm.sales_representative_email_address as sales_representative_email_address,
...
from analytics.prospects.prospects__mapping__v3 as pm
left join analytics.prospects.prospects__to__existing__companies__mapping__v3 as pex
...
###### added by support analyst for ticket #314532
  ...
  dimension: create_note {
    ...
    html:
    {% if is_prospect_note._value == 'Yes' %}
    <font color="blue "><u><a href = "https://app.seekwell.io/form/9b513151952049c3931bca19b8968fdf?timestamp={{ "now" | date: "%Y-%m-%d %H:%M" }}&prospect_id={{company_prospect_id._value }}&sales_representative_email_address={{  _user_attributes['email'] }}" target="_blank">Create Note</a></font></u>
    {% else %}
    <font color="blue "><u><a href = "https://docs.google.com/forms/d/e/1FAIpQLSc9BH1zaMNFdfHcBWKVi6I3ib-6QHwcYjffYqd8zlhX1zYgvg/viewform?usp=pp_url&entry.503989311={{ company_prospect_id._value }}&entry.1734875336={{ users.passing_user_id_from_logged_in_looker_user._value }}&entry.626077242=Sales" target="_blank">Submit Note</a></font></u>
    {% endif %} ;;
    sql:  ${company_prospect_id};;
  }
  ...
  dimension: merge_prospect {
    ...
    html:
    {% if is_prospect_note._value == 'Yes' %}
    <font color="blue "><u><a href = "https://app.seekwell.io/form/2c42a2e1800149ed9f2811cccf0e4f31?timestamp={{ "now" | date: "%Y-%m-%d %H:%M" }}&prospect_id={{company_prospect_id._value }}&sales_representative_email_address={{  _user_attributes['email'] }}" target="_blank">Merge Prospect</a></font></u>
    {% else %}
  ...
  dimension: reassign_prospect {
    ...
    html:
    {% if is_prospect_note._value == 'Yes' %}
   <font color="blue "><u><a href = "https://app.seekwell.io/form/169d80bf8daf4565818258f9e60ca60b?prospect_id={{company_prospect_id._value }}&sales_representative_email_address={{  _user_attributes['email'] }}" target="_blank">Update Prospect</a></font></u>
    {% else %}
    <font color="blue "><u><a href = "https://www.equipmentshare.com/" target="_blank"></a></font></u>
    {% endif %} ;;
    sql: ${company_prospect_id};;
  ...
  dimension: create_prospect_v2 {
    type: string
    html: <font color="blue "><u><a href = "https://app.seekwell.io/form/96ac96b7f9084e4da6755ff82447da28?sales_representative_email_address={{  _user_attributes['email'] }}" target="_blank">Create Prospect</a></font></u> ;;
    sql: ${company_prospect_id}  ;;
  }
