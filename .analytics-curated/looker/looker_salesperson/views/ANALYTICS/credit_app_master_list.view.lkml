view: credit_app_master_list {
  ...
  dimension: app_is_current_month {
    type: yesno
    sql: date_trunc(month,current_date()) =  date_trunc(month,${date_completed_raw}::DATE);;
  }

  dimension: app_is_previous_month {
    type: yesno
    sql: (date_trunc(month,current_date()) - interval '1 month') = date_trunc(month,${date_completed_raw}::DATE);;
  }
