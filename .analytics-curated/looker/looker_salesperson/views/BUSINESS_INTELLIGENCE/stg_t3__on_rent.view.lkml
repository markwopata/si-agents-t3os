view: stg_t3__on_rent {
    ...
    dimension: five_days_utilization_cst {
      group_label: "Five Days Utilization"
      type: number
    ...
    dimension: four_days_utilization_cst {
      group_label: "Four Days Utilization"
      type: number
    ...
    dimension: previous_day_utilization_cst {
      group_label: "Previous Day Utilization"
      type: number
  ...
  dimension: scheduled_off_rent_date_highlighted {
    ...
    dimension: seven_days_utilization_cst {
      group_label: "Seven Days Utilization"
      type: number
    ...
    dimension: six_days_utilization_cst {
      group_label: "Six Days Utilization"
      type: number
    ...
    dimension: three_days_utilization_cst {
      group_label: "Three Days Utilization"
      type: number
    ...
    dimension: two_days_utilization_cst {
      group_label: "Two Days Utilization"
      type: number
  ...
  dimension: primary_salesperson_name_location {
    label: "Primary Salesperson - Home Location"
    type: string
    sql: concat(${primary_salesperson_name}, ' - ', ${salesperson_permissions.employee_location}) ;;
  }

    dimension: bulk_flag {
      type: yesno
      sql: CASE WHEN ${make_and_model} not ilike '%bulk %' THEN FALSE
      WHEN ${make_and_model} not ilike '%bulk %' OR ${make_and_model} ilike '%bulk %' THEN TRUE END;;
    }
  ...
  measure: total_unfinished_pos {
    type: count_distinct
    sql: CASE WHEN ${po_highlight_check} THEN ${purchase_order_id} END ;;
    drill_fields: [company_name_id, rental_location, primary_salesperson_name_id, rental_id, order_id, asset_id, make_and_model, jobsite, purchase_order_id, purchase_order_name]
  }
  }
