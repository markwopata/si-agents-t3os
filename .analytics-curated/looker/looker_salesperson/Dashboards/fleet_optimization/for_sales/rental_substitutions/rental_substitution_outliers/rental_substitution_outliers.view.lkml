view: rental_substitution_outliers {
  derived_table: {
    sql:
        WITH oec AS (
        SELECT
...
base_subs as (
      ...
      i.monthly_invoiced_floor_rate - i.monthly_rental_floor_rate AS monthly_floor_sub_margin,
      i.monthly_invoiced_bench_rate - i.monthly_rental_bench_rate  AS monthly_bench_sub_margin,
      i.monthly_invoiced_online_rate - i.monthly_rental_online_rate AS monthly_online_sub_margin,
      case when i.monthly_rental_bench_rate > i.monthly_invoiced_bench_rate then 1 else 0 end as negative_bench_margin_flag,
      (i.monthly_invoiced_bench_rate - i.monthly_rental_bench_rate) / NULLIF(i.monthly_rental_bench_rate, 0) AS monthly_bench_sub_margin_pct,
      (i.monthly_invoiced_bench_rate - i.monthly_rental_bench_rate) * (cycle_length / 28) AS est_margin_dollars,
      ...
      class_stats AS (
  SELECT
    rental_equipment_class_name,
    AVG(monthly_bench_sub_margin) AS class_avg_margin,
    STDDEV_SAMP(monthly_bench_sub_margin) AS class_margin_stddev
  FROM base_subs
  GROUP BY rental_equipment_class_name
),

branch_class_summary AS (
  SELECT
    bs.branch,
    bs.rental_equipment_class_name,
    COUNT(*) AS sub_count,
    AVG(bs.monthly_bench_sub_margin) AS branch_class_avg_margin,
    SUM(bs.monthly_bench_sub_margin) AS total_margin,
    cs.class_avg_margin,
    cs.class_margin_stddev,
    (AVG(bs.monthly_bench_sub_margin) - cs.class_avg_margin) / NULLIF(cs.class_margin_stddev, 0) AS z_score
  FROM base_subs bs
  JOIN class_stats cs
    ON bs.rental_equipment_class_name = cs.rental_equipment_class_name
  GROUP BY bs.branch, bs.rental_equipment_class_name, cs.class_avg_margin, cs.class_margin_stddev
),

sample_summary AS (
  SELECT COUNT(*) AS total_pairs_in_sample
  FROM branch_class_summary
  WHERE sub_count >= 20
)

SELECT
  bcs.*,
  ss.total_pairs_in_sample
FROM branch_class_summary bcs
JOIN sample_summary ss ON 1=1
WHERE bcs.sub_count >= 20
ORDER BY z_score ASC
  ...
  measure: total_count {
    type: sum
    sql: ${TABLE}.sub_count ;;
    label: "Total Substitution Count"
  }

  dimension: branch_class_avg_margin {
    type: number
    sql: ${TABLE}.branch_class_avg_margin ;;
    label: "Avg. Substitution Margin ($)"
  ...
  dimension: class_avg_margin {
    type: number
    sql: ${TABLE}.class_avg_margin ;;
    label: "Class-wide Avg. Margin ($)"
    value_format_name: usd
  }

  dimension: class_margin_stddev {
    type: number
    sql: ${TABLE}.class_margin_stddev ;;
    label: "Class Margin Std. Dev."
    value_format_name: usd
  }
  ...
  dimension: z_score {
    type: number
    sql: ${TABLE}.z_score ;;
    label: "Z-Score (Margin Deviation)"
  }

  dimension: total_pairs_in_sample {
    type: number
    sql: ${TABLE}.total_pairs_in_sample ;;
    hidden: yes
  }
  ...
  dimension: performance_band {
    ...
    sql:
      CASE
        WHEN ${z_score} < -2 THEN 'High Negative Outlier'
        WHEN ${z_score} < -1 THEN 'Moderate Negative'
        WHEN ${z_score} > 2 THEN 'High Positive Outlier'
        WHEN ${z_score} > 1 THEN 'Moderate Positive'
        ELSE 'Typical'
      END ;;
