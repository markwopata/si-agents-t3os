view: sales_rep_rank_overall_ytd {
  derived_table: {
    sql:
SELECT u.user_id                                                   AS sales_rep_id,
       CONCAT(u.first_name, ' ', u.last_name)                      AS sales_rep,
       ANY_VALUE(cd.market_id)                                     AS home_market_id,
       SUM(li.amount)                                              AS year_to_date_revenue,
       DENSE_RANK() OVER (ORDER BY SUM(li.amount) DESC NULLS LAST) AS overall_rank
 ...
 WHERE li.gl_billing_approved_date >= DATE_TRUNC('year', CURRENT_DATE())
 ...
 ORDER BY overall_rank;;
  ...
  dimension: year_to_date_revenue {
    type: number
    value_format_name: usd_0
    sql: ${TABLE}."YEAR_TO_DATE_REVENUE" ;;
  }
