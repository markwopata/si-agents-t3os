view: fleet_spend_forecast {
  derived_table: {
    sql:
SELECT POL.INVOICE_NUMBER,PO.VENDOR_ID,C1.NAME AS VENDOR_NAME,POL.INVOICE_DATE, COALESCE(NT.DAYS,30) AS NET_TERM_DAYS,
DATEADD(DAY,COALESCE(NT.DAYS,30),INVOICE_DATE) AS DUE_DATE,POL.ORDER_STATUS, POL.RECONCILIATION_STATUS,POL.FINANCE_STATUS,
A.COMPANY_ID AS COMPANY_ID, C2.NAME AS ASSET_OWNER,POT.PREFIX||'PO'||POL.COMPANY_PURCHASE_ORDER_ID||'-'||POL.COMPANY_PURCHASE_ORDER_LINE_ITEM_NUMBER AS ORDER_NUMBER,
...
--AND POL.RECONCILIATION_STATUS = 'Reconciled'
AND A.COMPANY_ID IN (1854,32367,31712,6954,55524,76482)
AND POL.DELETED_AT IS NULL
GROUP BY POL.INVOICE_NUMBER, PO.VENDOR_ID,C1.NAME,POL.INVOICE_DATE, NT.DAYS, POL.ORDER_STATUS, POL.RECONCILIATION_STATUS,POL.FINANCE_STATUS, A.COMPANY_ID,C2.NAME,
POT.PREFIX||'PO'||POL.COMPANY_PURCHASE_ORDER_ID||'-'||POL.COMPANY_PURCHASE_ORDER_LINE_ITEM_NUMBER
 ;;
  ...
  dimension: es_owner {
    type: string
    sql: CASE WHEN ${TABLE}.COMPANY_ID IN (32367,31712,155,1854) THEN 'ES' ELSE 'Non-ES' END ;;
  }
  ...
  set: net_price_details {
    fields: [
      order_number,invoice_number,vendor_name,reconciliation_status,invoice_date,net_term_days,due_date,order_status,finance_status,days_until_due,due_days_buckets_monthly,asset_owner,net_price
    ]
  }
