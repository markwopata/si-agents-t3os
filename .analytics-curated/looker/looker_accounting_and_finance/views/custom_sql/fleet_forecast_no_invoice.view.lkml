view: fleet_forecast_no_invoice {
  derived_table: {
    sql:
    SELECT PO.COMPANY_PURCHASE_ORDER_ID,POL.INVOICE_NUMBER,PO.VENDOR_ID,C1.NAME AS VENDOR_NAME,POL.RELEASE_DATE, COALESCE(NT.DAYS,30) AS NET_TERM_DAYS,
    DATEADD(DAY,COALESCE(NT.DAYS,30),RELEASE_DATE) AS DUE_DATE,POL.ORDER_STATUS, POL.RECONCILIATION_STATUS,POL.FINANCE_STATUS,
    ...
    AND POL.RELEASE_DATE IS NOT NULL
    AND POL.NET_PRICE IS NOT NULL AND POL.NET_PRICE <> 0
    AND DUE_DATE >= '2016-01-01'
    AND POL.DELETED_AT IS NULL
    GROUP BY PO.COMPANY_PURCHASE_ORDER_ID,POL.INVOICE_NUMBER, PO.VENDOR_ID,C1.NAME,POL.RELEASE_DATE, NT.DAYS,
  ...
  set: net_price_details {
    fields: [
      order_number,purchase_order_id,invoice_number,vendor_name,reconciliation_status,release_date,net_term_days,due_date,order_status,
