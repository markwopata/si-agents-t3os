view: users {
  ...
  dimension: passing_user_id_from_logged_in_looker_user {
    type: number
    sql: (select ${TABLE}.USER_ID from ES_WAREHOUSE.PUBLIC.users where ${TABLE}.EMAIL_ADDRESS = '{{ _user_attributes['email'] }}') ;;
  }

  dimension: correct_email_address {
    type: string
    sql: case
         when ${email_address} like '%mike.mcclannahan%' then 'mike@equipmentshare.com'
         when ${email_address} like '%jeff.evans@equipmentshare.com%' then 'j.evans@equipmentshare.com'
         when ${email_address} like '%websterbailey@equipmentshare.com%' then 'web.bailey@equipmentshare.com'
         else ${email_address}
         end ;;
  }
