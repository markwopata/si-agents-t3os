view: asset_utilization_rolling30 {
  ...
  -- Assets in inventory and status at the start of the time period
      ...
      where m.company_id = {{ _user_attributes['company_id'] }}
        ...
        and ES_WAREHOUSE.PUBLIC.overlaps(sai.date_start, sai.date_end, convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date - interval '30 days', convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date + interval '59 mins')
        and ES_WAREHOUSE.PUBLIC.overlaps(ais.date_start::date, ais.date_end::date, convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date - interval '30 days', convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date + interval '59 mins')
     )
    ...
    --Calculate rental revenue by invoice data
      ...
      where m.company_id = {{ _user_attributes['company_id'] }}
        and li.charge_id = 1
        and i.issue_date::date >= convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date - interval '30 days' and i.issue_date::date < convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date + interval '1 day'
  ...
  measure: count_utilization {
    ...
    drill_fields: [day_date, asset_class_customer_branch.branch, asset_id, asset_inventory_status, total_asset_count]
  }
  ...
  measure: financial_utilization {
    ...
    value_format_name: percent_1
    drill_fields: [day_date, asset_class_customer_branch.branch, asset_id, asset_inventory_status, tot_rental_rev_for_oec_assets, tot_asset_oec, financial_utilization]
  }
