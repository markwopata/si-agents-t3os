view: asset_utilization_7day {
      derived_table: {
        sql:
       WITH day_series AS (
        select distinct date_trunc('day', series::date) as day_date
        from table(es_warehouse.public.generate_series(
                                dateadd(day, -6, convert_timezone('{{ _user_attributes['user_timezone'] }}', current_date))::timestamp_tz,
      ...
      -- Assets in inventory and status at the start of the time period
          ...
          where m.company_id = {{ _user_attributes['company_id'] }}
            ...
            and ES_WAREHOUSE.PUBLIC.overlaps(sai.date_start, sai.date_end, convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date - interval '6 days', convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date + interval '59 mins')
            and ES_WAREHOUSE.PUBLIC.overlaps(ais.date_start::date, ais.date_end::date, convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date - interval '6 days', convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date + interval '59 mins')
    --and a.asset_id = 16078
       ...
       , days_asset_on_rent as (
        ...
        where m.company_id = {{ _user_attributes['company_id'] }}
          and li.charge_id = 1
          and i.issue_date::date >= convert_timezone('{{ _user_attributes['user_timezone'] }}','UTC', current_timestamp)::date - interval '6 days'
        ...
        group by asset_id, market_id
          ...
          from days_asset_on_rent inv
            left join tot_rev_7d r on inv.market_id = r.market_id and inv.asset_id = r.asset_id
  ;;
  ...
  dimension: billed_rental_rev {
    label: "Billed Rental Revenue (7 Days)"
    type: number
  ...
  measure: count_utilization {
    label: "Unit Utilization (7 Days)"
    type: number
    sql: sum(${days_asset_on_rent}) / (${total_asset_count} * 7) ;;
  ...
  measure: est_yearly_rental_rev_for_oec_assets {
    label: "Annualized Rental Revenue (Rolling 7 Days)"
    type: number
    sql: ${tot_rental_rev_for_oec_assets} * 365 / 7 ;;
    value_format_name: usd_0
  }

  measure: financial_utilization {
    label: "Financial Utilization (7 Days)"
    type: number
