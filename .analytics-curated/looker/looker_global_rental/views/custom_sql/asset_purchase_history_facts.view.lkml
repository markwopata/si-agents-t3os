...
view: asset_purchase_history_facts_intermediary {
  derived_table: {
    ...
    FROM ${asset_purchase_history_initial.SQL_TABLE_NAME}
      LEFT JOIN ES_WAREHOUSE.PUBLIC.asset_purchase_history ON asset_purchase_history_initial.original_purchase_id = asset_purchase_history.purchase_history_id
    ;;
  }
...
view: asset_purchase_history_facts_final {
  ...
  measure: unavailable_oec {
    ...
    drill_fields: [asset_id, asset_status_key_values.value, OEC]
  }
...
# #
#     # filters: {
#     #   field: companies.timezone
#     #   value: _user_attributes['user_timezone']
#     # }
...
#   }
  ...
  measure:  unavailable_oec_perc{
    ...
    type: number
    sql: (${asset_purchase_history_facts_final.unavailable_oec} / case when ${asset_purchase_history_facts_final.OEC} = 0 then null else ${asset_purchase_history_facts_final.OEC} end) * 100;;
    value_format: "0.0\%"
    drill_fields: [asset_id, asset_status_key_values.value, OEC, assets_inventory.asset_class, asset_class_customer_branch.branch]
  }
