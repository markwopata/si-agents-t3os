view: assets {
  ...
  dimension: custom_name {
    ...
    sql: coalesce(${TABLE}."CUSTOM_NAME",'No Asset Assigned') ;;
  }
  ...
  dimension: delek_asset_description {
    type: string
    sql: concat(${asset_class},' ',${make},' ',${model}) ;;
  }
  ...
  dimension: custom_name_make_model {
    label: "Asset"
    group_label: "Asset Naming"
    group_item_label: "Link to Rental with Custom Name, Make, Model"
    type: string
    sql: concat(${custom_name},' ','(',${make},' ',${model},')') ;;
    html: <font color="#0063f3"><u><a href="https://app.estrack.com/#/assets/all/rentals/{{ rentals.rental_id._value }}/overview" target="_blank">{{value}}</a></font></u> ;;
  }

  dimension: custom_name_make_model_bulk_item {
    label: "Asset"
    group_label: "Asset Naming"
    group_item_label: "Link to Rental with Custom Name, Make, Model with Bulk Item"
    type: string
    sql: coalesce(concat(${custom_name},' ','(',${make},' ',${model},')'),'Bulk Item') ;;
    html: <font color="#0063f3"><u><a href="https://app.estrack.com/#/assets/all/rentals/{{ rentals.rental_id._value }}/overview" target="_blank">{{value}}</a></font></u> ;;
  }

  dimension: custom_name_make_model_bulk_item_no_link {
    label: "Asset"
    group_label: "Asset Naming"
    group_item_label: "Custom Name, Make, Model with Bulk Item"
    type: string
    sql: coalesce(concat(${custom_name},' ','(',${make},' ',${model},')'),'Bulk Item') ;;
  }

  dimension: custom_name_make_model_no_link {
    label: "Asset"
    group_label: "Asset Naming"
    group_item_label: "No Link Custom Name, Make, Model"
    type: string
    sql: concat(${custom_name},' ','(',${make},' ',${model},')') ;;
  }
