view: tcco_custom_invoice_export {
derived_table: {
...
select
  coalesce(csub.name, j.name, 'No Job/Sub Renter Listed') as "Job #"
, coalesce(subpo.name ,po.name) as "PO #"
...
, COALESCE(ai.asset_class, pt.description, ai.model, lis.description, lis.name) AS "Equipment Type"
...
--change from emma 2026/02/27
, i.start_date::date as "Date Rented"
, i.end_date::date as "Date Returned"
...
, coalesce(concat(usub.first_name, ' ',usub.last_name), concat(u.first_name, ' ',u.last_name)) as "Ordered By"
from
es_warehouse.public.invoices i
left join es_warehouse.public.orders o on o.order_id = i.order_id
...
left join es_warehouse.public.users usub on usub.user_id = sb.SUB_RENTER_ORDERED_BY_ID
...
left join es_warehouse.public.purchase_orders subpo on sb.SUB_RENTER_PURCHASE_ORDER_ID = subpo.purchase_order_id
-- left join
--           (
