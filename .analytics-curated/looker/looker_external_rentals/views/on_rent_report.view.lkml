...
, LAST_CYCLE_DATE
, NEXT_CYCLE_DATE
, coalesce(TOTAL_DAYS_ON_RENT, datediff('day',rental_start_date,current_date)) as TOTAL_DAYS_ON_RENT
-- Need COALESCE for `Bulk Items`
, TOTAL_DAYS_USED_ON_RENT
, TOTAL_DAYS_NOT_USED_ON_RENT
, TOTAL_WEEKDAYS_ON_RENT
, TOTAL_WEEKDAYS_USED_ON_RENT
, TOTAL_WEEKDAYS_NOT_USED_ON_RENT
, BILLING_DAYS_LEFT
...
, onr.sub_renting_company
, onr.sub_renting_contact
, 'On Rent' as rental_status
from
BUSINESS_INTELLIGENCE.TRIAGE.STG_T3__ON_RENT onr
where
onr.company_id = {{ _user_attributes['company_id'] }}
OR onr.company_id IN (SELECT DISTINCT COMPANY_ID FROM business_intelligence.triage.stg_t3__national_account_assignments
...
;;
  ...
  dimension: last_cycle_date_time {
    type: date_time
    sql: ${TABLE}."LAST_CYCLE_DATE" ;;
  }

  dimension: next_cycle_date_time {
    type: date_time
  ...
  dimension: total_days_used_on_rent {
    label: "Days Used on Rent"
    type: number
    sql: ${TABLE}."TOTAL_DAYS_USED_ON_RENT" ;;
  }

  dimension: total_days_not_used_on_rent {
    label: "Days Not Used on Rent"
    type: number
    sql: ${TABLE}."TOTAL_DAYS_NOT_USED_ON_RENT" ;;
  }

  dimension: total_weekdays_on_rent {
    label: "Weekdays on Rent"
    type: number
    sql: ${TABLE}."TOTAL_WEEKDAYS_ON_RENT" ;;
  }

  dimension: total_weekdays_used_on_rent {
    label: "Weekdays Used on Rent"
    type: number
    sql: ${TABLE}."TOTAL_WEEKDAYS_USED_ON_RENT" ;;
  }

  dimension: total_weekdays_not_used_on_rent {
    label: "Weekdays Not Used on Rent"
    type: number
    sql: ${TABLE}."TOTAL_WEEKDAYS_NOT_USED_ON_RENT" ;;
  }
  ...
  dimension: price_per_day {
    group_label: "Price Per Calculations (Rounded)"
    label: "Price Per Day"
    type: number
  ...
  dimension: price_per_day_raw {
    group_label: "Price Per Calculations (Raw)"
    label: "Price Per Day"
    type: number
    sql: ${TABLE}."PRICE_PER_DAY" ;;
  ...
  dimension: last_use_date {
    type: date
    sql: ${TABLE}."LAST_USE_DATE" ;;
  }

  dimension: days_since_last_use {
    type: number
    sql: ${TABLE}."DAYS_SINCE_LAST_USE" ;;
  ...
  measure: rental_period_utilization_hours_sum {
    label: "Total Rental Period Hours"
    type: sum
  ...
  dimension: link_to_asset_t3_bulk_asset {
    type: string
    sql: case when ${TABLE}."CUSTOM_NAME" = 'Asset to be assigned' then ${TABLE}."MAKE_AND_MODEL" else ${TABLE}."CUSTOM_NAME" end;;
    label: "Asset"
    group_label: "Link to T3 Status Page (Bulk Asset)"
    html: <font color="#0063f3"><u><a href="https://app.estrack.com/#/assets/all/asset/{{ asset_id._filterable_value }}/status" target="_blank">{{value}}</a></font></u>;;
  }
...
<td><b>
  ...
  measure: last_7_days_rental_period_on_time_hours {
    type: number
    sql:
    SUM(
      (CASE WHEN CAST(${previous_day_date} AS TIMESTAMP)
                >= ${rental_start_date_and_time_formatted}
            THEN COALESCE(${previous_day_on_time_hours},0) ELSE 0 END) +
      (CASE WHEN CAST(${two_days_ago_date} AS TIMESTAMP)
                >= ${rental_start_date_and_time_formatted}
            THEN COALESCE(${two_days_ago_on_time_hours},0) ELSE 0 END) +
      (CASE WHEN CAST(${three_days_ago_date} AS TIMESTAMP)
                >= ${rental_start_date_and_time_formatted}
            THEN COALESCE(${three_days_ago_on_time_hours},0) ELSE 0 END) +
      (CASE WHEN CAST(${four_days_ago_date} AS TIMESTAMP)
                >= ${rental_start_date_and_time_formatted}
            THEN COALESCE(${four_days_ago_on_time_hours},0) ELSE 0 END) +
      (CASE WHEN CAST(${five_days_ago_date} AS TIMESTAMP)
                >= ${rental_start_date_and_time_formatted}
            THEN COALESCE(${five_days_ago_on_time_hours},0) ELSE 0 END) +
      (CASE WHEN CAST(${six_days_ago_date} AS TIMESTAMP)
                >= ${rental_start_date_and_time_formatted}
            THEN COALESCE(${six_days_ago_on_time_hours},0) ELSE 0 END) +
      (CASE WHEN CAST(${seven_days_ago_date} AS TIMESTAMP)
                >= ${rental_start_date_and_time_formatted}
            THEN COALESCE(${seven_days_ago_on_time_hours},0) ELSE 0 END)
    )
  ;;
    html: {{rendered_value}} hrs. ;;
    value_format_name: decimal_1
