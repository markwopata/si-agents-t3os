...
-- test in Studio using below command
-- dbt build --select stg_t3__by_day_utilization --vars '{"row_limit": "LIMIT 10", "assets_table_slicer": "and a.asset_id = 3137"}' 
...
with
    date_series_raw as (
        ...
        where
            dt_date >= dateadd(day, -730, current_date()) and dt_date <= current_date()
    ),
    date_filterd_hau as (
        select
        ...
        where start_range >= dateadd(day, -730, current_date()) {{ var("row_limit") }}
    ),
    ...
    date_series as (
        select distinct
            ...
            coalesce(oa.OPERATOR_NAME, '') as operator_name
        ...
        left join 
            {{ ref("stg_t3__operator_assignments") }} oa
            on oa.asset_id = dai.asset_id
            and dsr.day >= oa.assignment_time::date
            and (oa.unassignment_time is null or dsr.day <= oa.unassignment_time::date)
    ),
    last_locations as (
        ...
        where start_date >= dateadd(day, -730, current_date())
        union
        select
            al.asset_id,
    ...
    odometer_daily as (
        ...
        where odo.date_start >= dateadd(day, -730, current_date())
    ...
    hours_daily as (
        ...
        where hrs.date_start >= dateadd(day, -730, current_date())
        qualify last_for_day = 1
    ),
    max_trip_start_times as (
        ...
        where
            t.start_timestamp >= dateadd(day, -730, current_date())
    ...
    -- link model & power type. emissions factor & power id 
    , model_power_emissions_factors AS (
        SELECT
        e.model_id,
        e.power_type_id,
        p.power_type,
        ef.EQUIPMENT_POWER_TYPE_ID AS power_type_id,
        ef.EMISSIONS_FACTOR AS kg_co2_per_gallon
        from TAXES.CHARGES.EQUIPMENT e 
        LEFT JOIN TAXES.CHARGES.EQUIPMENT_POWER_TYPES p
        ON e.power_type_id = p.equipment_power_type_id
        LEFT JOIN EXPERIENCE.PUBLIC.EQUIPMENT_EMISSIONS_FACTOR AS ef
        ON ef.EQUIPMENT_POWER_TYPE_ID = e.power_type_id
        )
  ...
  ,  cleaned_trips as (
    ...
    in_progress_trips as (
        ...
        left join {{ ref("stg_t3__operator_assignments") }} oa
            on oa.ASSET_ID = t.asset_id
            and coalesce(t.end_timestamp, current_timestamp()) >= oa.ASSIGNMENT_TIME
            and coalesce(t.end_timestamp, current_timestamp()) <= oa.UNASSIGNMENT_TIME
    ...
    utc_time as (
        ...
        left join {{ ref("stg_t3__operator_assignments") }} oa
            on oa.ASSET_ID = hau.asset_id
            and coalesce(hau.end_range, current_timestamp()) >= oa.ASSIGNMENT_TIME
            and coalesce(hau.end_range, current_timestamp()) <= oa.UNASSIGNMENT_TIME
        group by
            start_range::date,
            hau.asset_id,
    ...
    data_layer as (
        ...
        left join
            ...
            and coalesce(utc.OPERATOR_NAME, '') = coalesce(ds.OPERATOR_NAME, '')
        left join
            est_time est
            on est.date = ds.day
            ...
            and coalesce(est.OPERATOR_NAME, '') = coalesce(ds.OPERATOR_NAME, '')
        left join
            cst_time cst
            on cst.date = ds.day
            ...
            and coalesce(cst.OPERATOR_NAME, '') = coalesce(ds.OPERATOR_NAME, '')
        left join
            mnt_time mnt
            on mnt.date = ds.day
            ...
            and coalesce(mnt.OPERATOR_NAME, '') = coalesce(ds.OPERATOR_NAME, '')
        left join
            wst_time wst
            on wst.date = ds.day
            ...
            and coalesce(wst.OPERATOR_NAME, '') = coalesce(ds.OPERATOR_NAME, '')
        left join
    ...
    last_used_date as (
        ...
        from cst_time
        where run_time_cst > 0
        group by asset_id, rental_company_id, owner_company_id
    )
select distinct
    concat(dl.date, dl.asset_id, dl.rental_company_id, coalesce(dl.owner_company_id, a.company_id, null), coalesce(cv.rental_id,0), coalesce(cv.order_id,0), dl.job_id, dl.phase_job_id, coalesce(dl.OPERATOR_NAME, ai.driver_name, ''))    as primary_key,
    dl.date,
    ...
    cv.rental_parent_company_id,
    cv.rental_parent_company_name,
    cv.owner_parent_company_id,
    cv.owner_parent_company_name,
    on_time_utc,
    in_progress_on_time_utc,
    run_time_utc,
    idle_time_utc,
  ...
  --  jobsite,
    ...
    ai.category,
    case
        when ai.company_id = 1854
        or dl.asset_id in (select pp.asset_id from ES_WAREHOUSE.PUBLIC.V_PAYOUT_PROGRAMS pp where pp.end_date is null) then mo.name
        else ai.branch
    end as branch,
    ai.make,
    ai.model,
    ...
    ai.esdb_tracker_id,
    coalesce(dl.OPERATOR_NAME, ai.driver_name) as driver_name,
    ...
    TRIM(REGEXP_REPLACE(c.name, '\t+', ' ')) as rental_company_name,

    coalesce(mpe.power_type, 'Unknown') as engine_power_type,
    coalesce(mpe.kg_co2_per_gallon,0) as kg_co2_per_gallon,

    coalesce((mpe.kg_co2_per_gallon * afc.gallons_used_per_day),0) as emissions_per_day,
    coalesce((mpe.kg_co2_per_gallon * afc.idle_gallons_per_day),0) as idle_emissions_per_day,

    case when mpe.power_type is null then 'Unknown' else 'Estimate' end as emissions_calculation_type,
    ...
    datediff(day, lud.last_used_date, dl.date) as days_since_last_used,
...
left join {{ ref("platform", "es_warehouse__public__markets") }}  mo on mo.market_id = a.rental_branch_id
...
left join BUSINESS_INTELLIGENCE.TRIAGE.STG_T3__ASSET_FUEL_CONSUMPTION afc on afc.asset_id = dl.asset_id and dl.date = afc.start_date and afc.job_id = dl.job_id 
left join model_power_emissions_factors mpe on a.equipment_model_id = mpe.model_id 

--left join BUSINESS_INTELLIGENCE.TRIAGE.stg_t3__national_account_assignments pcr on pcr.company_id = o.company_id
left join {{ ref('stg_t3__company_values') }} cv
            on dl.asset_id = cv.asset_id
            and dl.date >= cv.start_date::date
            and dl.date <= cv.end_date::date
            and dl.rental_company_id = cv.rental_company_id
            and dl.owner_company_id = cv.owner_company_id
left join es_warehouse.public.companies c 
    on c.company_id = dl.rental_company_id
left join
...
left join
    last_used_date lud
    on lud.asset_id = dl.asset_id
    and  coalesce(lud.rental_company_id,0) = coalesce(dl.rental_company_id, 0)
    and  coalesce(lud.owner_company_id,0) = coalesce(dl.owner_company_id, 0)
    -- left join
