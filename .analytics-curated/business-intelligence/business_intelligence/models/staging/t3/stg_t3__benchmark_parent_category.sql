...
select
    ...
    on_time_hours / (8 * rental_date_count) as utilization_30_day_parent_category_benchmark
...
join {{ ref("stg_t3__asset_info") }} ai on ai.asset_id = bdu.asset_id
where bdu.date >= dateadd(day, -30, current_date())
and ai.parent_category is not null
group by ai.parent_category
order by
